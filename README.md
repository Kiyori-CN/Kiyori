...
应用启动后进入软件首页。负一屏、软件首页和 AI 首页共享同一 Pager 状态、释放速度与吸附物理；页面跟随手指移动，未结束的滑动可被立即反向手势中断。软件首页在手机使用居中单列，在 `600dp` 平板和 `840dp` 大屏使用受限宽度的品牌/搜索双区布局。搜索卡保留独立的“搜索”和“AI”按钮：“搜索”以淡入和轻微上移转场打开只负责网页搜索的全屏页，“AI”进入右侧 AI 首页。全屏页自动聚焦输入框，直接复用浏览器的搜索引擎和最近记录；右侧无痕按钮切换 Browser Runtime 的新窗口默认 Profile，启用后本页提交与未显式指定 Profile 的 AI 新建窗口都会创建无痕 WebSession。提交网址或关键词会创建并激活新的共享 WebSession，再进入 Browser Home，因此不会覆盖用户当前正在浏览的窗口，AI 也能立即通过 `browser_tabs list` 发现该窗口。浏览器顶栏搜索仍只导航当前窗口。历史、书签、文件和小程序分别在自己的页面中提供搜索。底部五入口只在五个根页面显示，负一屏和保持全屏形态的 AI 首页隐藏底栏。AI 首页即使尚未配置模型或 API Key，也始终显示对话界面和输入框；模型配置由独立设置页承载，缺少凭据不会阻塞页面进入。AI 首页及 AI 一级根页面的左上角三横线按钮打开模态 AI 左抽屉，深层页面改为返回按钮；从快捷方式或外部入口直达一级根页面时也遵守同一规则。AI 设置也可从设置首页进入同一页面，跨来源进入时从设置根页开始，避免继承另一个入口的深层返回路径。

浏览器首页、浏览器悬浮窗和 Operit AI 浏览器工具使用同一进程内 WebSession。三者共用标签、活动 WebView、历史、书签、下载和用户脚本；展示位置切换只转挂同一个 WebView。每个窗口持有不可变的 `normal` 或 `incognito` Profile：普通窗口继续使用默认 WebView Profile，同一次无痕会话的窗口共享 Kiyori 私有 AndroidX Profile；最后一个无痕 WebView 销毁后该 Profile 被删除。网页 Cookie、WebStorage、userscript Cookie/XHR 和下载请求均跟随窗口 Profile，普通与无痕之间不复制网站数据。悬浮浏览器展开后，背景延伸到状态栏和挖孔区域；系统 Back 与左上角返回都先按同一浏览器状态机关闭当前浮层，再把网页收缩为悬浮球，不会让网页后退。AI 可通过 `browser_tabs list` 看到人工打开的全部窗口及其稳定 `session_id`、`profile`、标题、网址和活动状态，也可用 `browser_tabs create profile=normal|incognito` 显式新建，再继续读取快照、点击和输入。无痕只隔离网站数据，不隔离用户已授权的 Kiyori AI 操作。浏览器窗口页顶部只保留普通/无痕 selector，按手机和平板宽度使用 2、3、4 列，并显示 `postVisualStateCallback` 驱动、仅驻留内存的网页缩略图；新建与清空只作用于当前 selector。设备 WebView 不支持 Multi-Profile 或无法安全清理旧无痕 Profile 时，入口会明确禁用，不会创建普通窗口代替。从 AI 首页顶栏打开 Browser Home 后，浏览器返回会回到 AI 首页并恢复悬浮球。浏览器底栏第五按钮是固定四行的浏览器菜单，历史、下载、书签、插件、UA、网络日志、源码和能力说明页使用可拖动子抽屉；菜单“无痕模式”进入真实窗口页，“AI 对话”回到 AI 首页，“退出浏览器”只移除展示层，清空全部窗口才清理 session。首期继续使用设备的 Android System WebView provider，新下载写入 `Download/Kiyori/browser/downloads/`。

设置首页只显示已经接入真实状态和页面的入口，首期为“网页浏览器”和“Operit AI 设置”；下载器、播放器和其他产品设置会在对应能力完成时再加入。网页浏览器设置从设置首页、Browser Home 菜单或悬浮浏览器菜单进入同一页面，共用搜索引擎、搜索记录、新窗口默认普通/无痕 Profile 和桌面/手机 UA 状态。搜索记录页支持逐条删除和清空；系统 Back 与顶栏返回会按“搜索记录 → 浏览器设置 → 原设置页或浏览器页”的顺序回退。普通网站数据继续保留，无痕网站数据遵守最后一个无痕窗口关闭后删除的生命周期。
