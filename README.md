...
页面的“当前路由”会把当前订阅的“默认代理”根组展开到实际组选择和最终节点；自动组没有可用选择或
发生循环引用时会明确显示未解析状态，不会把组名伪装成节点。逐脚本抽屉只列出已启用且可执行的传统
JsEngine 包，同时保留手动输入包名的入口；没有脚本时代表当前没有可配置的传统脚本，不影响 AI 工具模块。
