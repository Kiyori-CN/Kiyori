...
- **内容管理**：广告订阅、自定义 URL 与元素规则、“标记广告”工作台、用户脚本和浏览器插件中心；支持 AI 协助开发项目兼容的 `.kbx` 扩展。
...
文件管理器提供双栏浏览、搜索与选择、复制和移动、回收站、ZIP、书签和本地预览。
两栏独立保留目录与选择，设置往返和应用内最小化保留当前会话；显示偏好与历史在对应设置中管理。
手机存储、Ubuntu、SAF 与网络目录的可用操作不同，使用前按实际能力确认。
...
| 应用标识 | `com.kiyori` |
| 版本 | 以下载页说明、[构建配置](app/build.gradle.kts) 与实际 APK 为准 |
...
当前工具权限提供允许、每次询问和禁止，以及单工具例外。风险分级与逐动作确认是进一步的设计目标，
尚不能保证所有工具中的高影响操作都有独立确认；请按工具能力和实际提示配置权限。
现有实现与目标设计的区别见 [工具授权契约](docs/doc-src/contracts/extensions_workspace.md#应用内-ai-动作授权)。
...
修改文档前请阅读[文档维护规范](docs/doc-src/documentation_guide.md)。尤其不要把一次性排障记录、
