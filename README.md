...
以下结构已经确定为正式开发目标。首个 App Shell 切片已经接管启动页、三页首页空间、五个根入口和底栏显示规则；AI 导航使用覆盖当前页面的模态左抽屉。浏览器首页首期已接入人与 AI 共用的 WebSession，软件首页、全屏网页搜索、AndroidX Multi-Profile 无痕窗口和内存网页缩略图也已接入同一 Browser Runtime。负一屏、文件管理首页和设置首页已经按固定旧版参考提交完成布局复刻；文件分类计数仍为占位值，手机存储已显示应用所在数据卷的实际可用与总容量。设置首页当前已接通账号与连接、AI 助手、语音服务、网页浏览器、视频播放器、文件下载器、界面定制和数据备份与同步；小程序管理只为底部第三个小程序产品域保留空入口，不连接“扩展”。Kiyori 主题系统保留固定浅色、深色与跟随系统模式，不允许用户颜色重写整个应用；AI 对话背景、气泡、头像、聊天头部和输入区仍可局部个性化。小程序首页仍是接线骨架，真机验收尚未完成：
...
上述 AI 设置入口当前统一显示为“AI 助手”。它只承载模型与 API、功能模型、用户偏好、提示词、人设、分句回复、上下文、AI 工具授权、Token 统计和外部 HTTP 对话；“扩展”、MCP、Skill、ToolPkg 和工作流继续留在各自 AI 导航目的地，不进入设置首页的“小程序管理”。
...
浏览器菜单的“插件”打开 Browser Plugin Center，不进入 AI 对话页“扩展”。插件中心提供“本页 / 已安装”、
...
模态 AI 左抽屉、“扩展”、权限、工作流、AI 对话和通用状态控件继续共用
...
抽屉提供动态网络状态、“扩展”、权限授予、工作流、AI 对话、助手配置、记忆库、工具箱、ToolPkg 动态插件和 AI 助手；WiFi 网络显示为“WiFi”。“扩展”、权限和工作流三张快捷卡为右上角数量或状态徽标保留独立顶部区域，“正常”等文字不会覆盖居中的彩色图标。“扩展”固定按“脚本 / 插件 / Skill / MCP”显示四个标签，首次进入默认打开“脚本”：“插件”管理 ToolPkg 容器，包括随 APK 预置和从市场安装的插件；普通 JS、TS 与 HJSON 工具项目显示在“脚本”标签，其导入文件和错误提示继续使用“脚本包”这一技术名称。脚本包类型统一按英文名称 A 到 Z 分组，类型内按工具包显示名的英文或中文拼音首字母排序；相同首字母继续使用简体中文校对顺序和内部包名形成稳定次级顺序。当前 17 个登记类型各自拥有不同的浅深主题颜色组合和匹配含义的图标，自定义类型使用稳定生成的分类身份。

“扩展”在宿主顶栏显示同名标题。插件和脚本标签的页面级动作按“环境变量 / 市场 / 添加 / 刷新”排列；Skill 按条件显示错误入口，并固定提供“市场 / 添加 / 刷新”；MCP 固定提供“启动 / 市场 / 添加 / 刷新”。这些动作使用同一视觉重量的细线图标，只有错误、成功、分类和运行状态保留明确的状态表达。Skill 与 MCP 仍各自持有弹窗、加载状态、仓库和执行回调；顶栏只投影当前活动标签的动作，加载时禁用冲突入口，并只在所属动作显示进度。四个标签不再保留页面级右下角浮动按钮，Skill 卡片内也不再重复显示刷新入口。

搜索框单独固定在顶栏下方，四个标签固定在搜索框下方，只有列表内容滚动。脚本列表顶部不显示“快速创作”宣传卡片；脚本标签的加号菜单固定提供“创建脚本 / 导入脚本包”，插件标签的加号继续直接导入插件。“创建脚本”只要求描述目标并进入既有创作流程，不再显示或执行旧的快速配置步骤。Skill 的三个导入标签与 MCP 的四个导入标签始终显示正确指示器。插件和脚本加载错误显示为标签下方的紧凑提示条。手动刷新、市场成功安装脚本或 ToolPkg、删除包和删除冲突源共用同一个完整包快照重载入口，市场安装后不必重启应用。环境变量顶栏入口打开与浏览器子页面同源的三态可拖动底部抽屉，顶部固定显示总览、搜索和同样按 A 到 Z 排列的 `ToolPackage.category` 类型条，内容按类型和工具包显示名折叠分组；包头使用 `32dp` 类型图标和紧凑两行文字，展开变量使用 `42dp` 单行输入区，底部取消与保存不随长列表滚动。每次打开抽屉都会依据最新已保存值重新计算初始状态：自动展开所有存在未填写必填变量的工具包，必填项完整或只有可选变量的工具包默认收起。变量声明和保存继续由唯一 `ToolPackage.env` 与 `EnvPreferences` 持有。插件继续使用 Operit 市场的 `package/toolpkg_v2` 协议，并可注册抽屉页面、输入框菜单、消息处理和配置页面。这些 AI 包与插件入口不会映射到设置首页的“小程序管理”。原生 AI 一级根保留独立状态与子栈，ToolPkg 页面只在其路由声明 `keepAlive=true` 时保留状态。对话历史、新建和删除对话继续由 AI 首页原有历史选择器负责，不复制到抽屉；Terminal 仍只保留在 AI 首页右上角。负一屏按旧 Kiyori 固定布局展示收藏、书签、历史、下载四张数据卡，以及新版、手册、版本、搜索、工具箱、清理、备份、退出八个空动作快捷工具；书签、历史和下载卡显示各自真实数量并打开与浏览器菜单相同的共享抽屉，收藏保留零计数和空点击，专用于未来小程序服务。
