...
以下结构已经确定为正式开发目标。首个 App Shell 切片已经接管启动页、三页首页空间、五个根入口和底栏显示规则；AI 导航使用覆盖当前页面的模态左抽屉。浏览器首页首期已接入人与 AI 共用的 WebSession，软件首页、全屏网页搜索、AndroidX Multi-Profile 无痕窗口和内存网页缩略图也已接入同一 Browser Runtime，真机验收尚未完成；小程序、文件管理、设置和负一屏数据仍在分阶段接入，不表示这些页面已经全部实现：
...
应用启动后进入软件首页。负一屏、软件首页和 AI 首页共享同一 Pager 状态、释放速度与吸附物理；页面跟随手指移动，未结束的滑动可被立即反向手势中断。软件首页在手机、平板和横屏都保持“标题在上、搜索框在下”的居中单列：常规高度把搜索框顶部描边精确放在可用首页高度的 `38.2%`，短高度按标题完整可见所需的最小顶部位置布置；Compact、Medium 和 Expanded 的内容宽度上限分别为 `544dp`、`584dp` 与 `624dp`，对应最小水平留白为 `24dp`、`48dp` 与 `72dp`。标题右上方显示约 `18dp` 的小圆环标志，搜索框常规高 `114dp`、短布局高 `96dp`，内部与页面同色，只用约 `1dp` 的彩色渐变线定义边界，不绘制外侧扩散、模糊或阴影。搜索与 AI 位于同一个 `120×32dp` 圆角分段框内，左右严格等宽，文字和选项图标不随边框缩小；选中项使用蓝色图标、文字和轻量圆角底，按压水波也限制在圆角选区内。选项本身不导航；点击搜索框主体时，搜索模式打开全屏网页搜索，AI 模式进入现有 AI 首页、保留会话和草稿，并在页面稳定后聚焦真实输入框和弹出输入法。模式在返回首页和旋转后保持，冷启动默认搜索。附件、语音和相机仍是右下角立即动作，先进入 AI 首页，再分别调用现有附件面板、全屏语音会话和相机拍照流程，单次点击只消费一次。
...
全屏网页搜索自动聚焦输入框，直接复用浏览器的搜索引擎和普通模式最近记录。软件首页和浏览器顶栏的两个入口共用右侧浏览模式控件：睁眼表示有痕模式，闭眼表示无痕模式；图标按钮不绘制外框、底色或阴影，切换后只在导航栏上方短时显示白底黑字的“已开启无痕模式”或“已关闭无痕模式”，不展示常驻长说明。该控件同步 Browser Runtime 的新窗口默认 Profile；软件首页提交始终创建并激活新的共享 WebSession，再进入 Browser Home，因此不会覆盖用户当前正在浏览的窗口。浏览器顶栏提交在所选 Profile 与活动窗口一致时继续当前窗口，不一致时创建对应 Profile 的新窗口。AI 能立即通过 `browser_tabs list` 发现这些窗口。历史、书签、文件和小程序分别在自己的页面中提供搜索。底部五入口只在五个根页面显示，负一屏和保持全屏形态的 AI 首页隐藏底栏。AI 首页即使尚未配置模型或 API Key，也始终显示对话界面和输入框；模型配置由独立设置页承载，缺少凭据不会阻塞页面进入。AI 首页及 AI 一级根页面的左上角三横线按钮打开模态 AI 左抽屉，深层页面改为返回按钮；从快捷方式或外部入口直达一级根页面时也遵守同一规则。AI 设置也可从设置首页进入同一页面，跨来源进入时从设置根页开始，避免继承另一个入口的深层返回路径。

浏览器首页、浏览器悬浮窗和 Operit AI 浏览器工具使用同一进程内 WebSession。三者共用标签、活动 WebView、历史、书签、下载和用户脚本；展示位置切换只转挂同一个 WebView。每个窗口持有不可变的 `normal` 或 `incognito` Profile：普通窗口继续使用默认 WebView Profile，同一段无痕会话的窗口共享唯一命名的 Kiyori 私有 AndroidX Profile 代际；最后一个无痕 WebView 销毁后清理 Cookie、WebStorage 和定位授权并立即退休该代际，后续无痕窗口使用全新代际。AndroidX 不允许在同一进程删除已加载 Profile，因此退休代际在下次冷启动、尚未重新加载时物理删除。网页 Cookie、缓存、WebStorage、userscript Cookie/XHR 和下载请求均跟随窗口 Profile，普通与无痕之间不复制网站数据；无痕搜索、访问和标题更新不写入共享历史，书签与下载仍保留用户明确发起的持久化语义。悬浮浏览器展开后，背景延伸到状态栏和挖孔区域；系统 Back 与左上角返回都先按同一浏览器状态机关闭当前浮层，再把网页收缩为悬浮球，不会让网页后退。AI 可通过 `browser_tabs list` 看到人工打开的全部窗口及其稳定 `session_id`、`profile`、标题、网址和活动状态，也可用 `browser_tabs create profile=normal|incognito` 显式新建，再继续读取快照、点击和输入。无痕只隔离网站数据，不隔离用户已授权的 Kiyori AI 操作。浏览器窗口页顶部只保留普通/无痕 selector，按手机和平板宽度使用 2、3、4 列，并显示 `postVisualStateCallback` 驱动、仅驻留内存的 `5:8` 纵向网页缩略图；捕获完整保留当前可见 WebView 视口，不裁切网页。新建与清空只作用于当前 selector。设备 WebView 不支持 Multi-Profile 或冷启动无法安全清理旧无痕 Profile 时，入口会明确禁用，不会创建普通窗口代替。从 AI 首页顶栏打开 Browser Home 后，浏览器返回会回到 AI 首页并恢复悬浮球。浏览器底栏第五按钮是固定四行的浏览器菜单，历史、下载、书签、插件、UA、网络日志、源码和能力说明页使用可拖动子抽屉；菜单“无痕模式”进入真实窗口页，“AI 对话”回到 AI 首页，“退出浏览器”只移除展示层，清空全部窗口才清理 session。首期继续使用设备的 Android System WebView provider，新下载写入 `Download/Kiyori/browser/downloads/`。

设置首页只显示已经接入真实状态和页面的入口，首期为“网页浏览器”和“Operit AI 设置”；下载器、播放器和其他产品设置会在对应能力完成时再加入。网页浏览器设置从设置首页、Browser Home 菜单或悬浮浏览器菜单进入同一页面，共用搜索引擎、搜索记录、新窗口默认普通/无痕 Profile 和桌面/手机 UA 状态。搜索记录页支持逐条删除和清空；系统 Back 与顶栏返回会按“搜索记录 → 浏览器设置 → 原设置页或浏览器页”的顺序回退。普通网站数据继续保留；无痕网站数据在最后一个无痕窗口关闭后立即停止复用，并在下次冷启动物理删除退休 Profile。
