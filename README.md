...
以下结构已经确定为正式开发目标。首个 App Shell 切片已经接管启动页、三页首页空间、五个根入口和底栏显示规则；AI 导航使用覆盖当前页面的模态左抽屉。浏览器首页首期已接入人与 AI 共用的 WebSession，软件首页与全屏网页搜索也已接入同一 Browser Runtime、搜索引擎和搜索记录，真机验收尚未完成；小程序、文件管理、设置和负一屏数据仍在分阶段接入，不表示这些页面已经全部实现：
...
应用启动后进入软件首页。负一屏、软件首页和 AI 首页共享同一 Pager 状态、释放速度与吸附物理；页面跟随手指移动，未结束的滑动可被立即反向手势中断。软件首页在手机使用居中单列，在 `600dp` 平板和 `840dp` 大屏使用受限宽度的品牌/搜索双区布局。搜索卡保留独立的“搜索”和“AI”按钮：“搜索”以淡入和轻微上移转场打开只负责网页搜索的全屏页，“AI”进入右侧 AI 首页。全屏页自动聚焦输入框，直接复用浏览器的搜索引擎和最近记录；提交网址或关键词会在共享 Browser Runtime 中创建并激活新 WebSession，再进入 Browser Home，因此不会覆盖用户当前正在浏览的窗口，AI 也能立即通过 `browser_tabs list` 发现该窗口。浏览器顶栏搜索仍只导航当前窗口。历史、书签、文件和小程序分别在自己的页面中提供搜索。底部五入口只在五个根页面显示，负一屏和保持全屏形态的 AI 首页隐藏底栏。AI 首页即使尚未配置模型或 API Key，也始终显示对话界面和输入框；模型配置由独立设置页承载，缺少凭据不会阻塞页面进入。AI 首页及 AI 一级根页面的左上角三横线按钮打开模态 AI 左抽屉，深层页面改为返回按钮；从快捷方式或外部入口直达一级根页面时也遵守同一规则。AI 设置也可从设置首页进入同一页面，跨来源进入时从设置根页开始，避免继承另一个入口的深层返回路径。

浏览器首页、浏览器悬浮窗和 Operit AI 浏览器工具使用同一进程内 WebSession。三者共用标签、活动 WebView、Cookie、历史、书签、下载和用户脚本；展示位置切换只转挂同一个 WebView。悬浮浏览器展开后，背景延伸到状态栏和挖孔区域；系统 Back 与左上角返回都先按同一浏览器状态机关闭当前浮层，再把网页收缩为悬浮球，不会让网页后退。AI 可通过 `browser_tabs list` 看到人工打开的全部窗口及其稳定 `session_id`、标题、网址和活动状态，选择窗口后继续读取快照、点击和输入。从 AI 首页顶栏打开 Browser Home 后，浏览器返回会回到 AI 首页并恢复悬浮球。浏览器底栏第五按钮是固定四行的浏览器菜单，历史、下载、书签、插件、UA、网络日志、源码和能力说明页使用可拖动子抽屉；菜单“AI 对话”回到 AI 首页，“退出浏览器”只移除展示层，清空全部窗口才清理 session。首期继续使用设备的 Android System WebView provider，新下载写入 `Download/Kiyori/browser/downloads/`。普通/无痕窗口页同时展示两种模式，但无痕 runtime 隔离尚未接入，不会用共享 Cookie 冒充无痕。
