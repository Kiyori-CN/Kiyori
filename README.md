...
以下结构已经确定为正式开发目标。首个 App Shell 切片已经接管启动页、三页首页空间、五个根入口和底栏显示规则；AI 导航使用覆盖当前页面的模态左抽屉。浏览器首页首期已接入人与 AI 共用的 WebSession，真机验收尚未完成；小程序、文件管理、设置、负一屏数据与网页搜索仍在分阶段接入，不表示这些页面已经全部实现：
