...
浏览器首页、浏览器悬浮窗和 Operit AI 浏览器工具使用同一进程内 WebSession。三者共用标签、活动 WebView、Cookie、历史、书签、下载和用户脚本；展示位置切换只转挂同一个 WebView。首期继续使用设备的 Android System WebView provider，新下载写入 `Download/Kiyori/browser/downloads/`。
