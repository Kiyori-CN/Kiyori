...
底栏第四项首页的“存储位置 → 全部”仅提供内部存储、Linux、工作区和回收站四个内置位置；
点击位置直接打开对应目录或回收站。内部存储显示可刷新容量，Linux 与工作区可切换首页和侧栏显示，
隐藏不删除文件，隐藏的入口仍可从“全部”页打开。该页不提供添加其他位置功能。
存储左抽屉通过按钮打开，点击遮罩或返回关闭，不再通过左滑收起。入口使用彩色图标；书签不再显示删除叉号。
长按网络、书签或工作区可编辑、删除入口、排序或创建桌面快捷方式；其他本地入口可重命名、删除、隐藏和排序，根目录与内部存储固定保留。
“设置 → 文件管理器”可配置左右列启动目录、显示密度、文件名行数、时间精度、目录大小和刷新间隔，管理侧栏显示与隐藏清单，并清理本机历史。
启动目录与初始排列用于新会话，其他显示偏好即时生效；编辑名称或删除侧栏入口不会重命名、移动或删除实际文件。
...
可见本地文件夹在后台统计内容大小，滚动和列表刷新保留已知数值；前台定期核对内容，大小变化后更新，手动刷新可立即重查。
无法访问、超时或不支持统计的位置明确显示“大小不可用”，可通过刷新重试。右侧拖柄在滚动时显示，停止后自动隐藏。
...
在“扩展 → 脚本”中，可横向滑动分类条选择“全部”或具体分类，并结合搜索查找工具；“环境变量”抽屉使用相同的分类条。
...
本 README 只保留面向用户和新贡献者的稳定入口。术语、状态所有者和不变量以
[`CONTEXT.md`](CONTEXT.md) 为准；维护流程、验证和交付规则以
[`AGENTS.md`](AGENTS.md) 为准；详细架构、协议和阶段证据分别放在
[`docs/doc-src/`](docs/doc-src/)、[`docs/user-guide/`](docs/user-guide/) 与
[`docs/TODO/`](docs/TODO/)。

修改文档前请阅读[文档维护规范](docs/doc-src/before_docing.md)。尤其不要把一次性排障记录、
临时构建数字或尚未验收的功能写成 README 的当前事实；需要补充用户行为时，先更新对应权威
文档，再在这里保留简短说明和链接。
