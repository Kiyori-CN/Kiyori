...
- **可切换的浏览器主页**：在“设置 → 网页浏览器 → 网页主页”中选择 Kiyori 原生主页、自定义网址或纯空白页。原生 Dashboard 直接投影共享书签、网页历史、活动标签页和搜索入口；官网验证、书签、历史和新标签继续走同一 Browser Runtime，不复制 WebView 或网页会话。
...
- **可选启动恢复**：网页浏览器设置提供边缘滑屏前进后退、恢复上次搜索结果、恢复前询问和保留普通多窗口四个独立开关；无痕窗口和无痕元数据不会写入恢复记录。当前开发验证阶段的新安装默认选择“自定义网址”，地址为 `https://web.gotab.cn/`；该设置也可切换为 Kiyori 原生主页或纯空白页。
