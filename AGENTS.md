...
## 执行准则
- 每轮完成仓库修改后，默认在收尾阶段串行执行 `./gradlew :app:assembleDebug --no-daemon --console=plain`，并核验生成的 Debug APK；用户当前明确要求跳过构建时除外。
- 除生成 Debug APK 所需任务外，额外的单元测试、Lint、Release 构建或其他专项检查只在用户明确要求或当前改动风险确有必要时执行。
- 默认不创建提交、不推送远端；只有用户在当前任务中明确要求时，才允许提交或推送。
