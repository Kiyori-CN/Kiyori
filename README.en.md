...
- **Extension ecosystem** for script packages, ToolPkg plugins, Skills, MCP servers, markets, environment variables, and permissions. Bundled scripts and plugins use normalized bilingual Agent-facing descriptions and categories; packages that declare environment variables start disabled on a fresh install. Search and Academic capabilities include OpenAI Search, Brave Search, arXiv, Crossref, PubMed, Semantic Scholar, and OpenAlex.
- **Built-in text to speech**: a first-time speech-profile migration enables the verified Next Chinese HTTP voice with the Xiaoxiao voice on a new install; an existing installation keeps its stored TTS configuration. Change it under Settings Home → AI Assistant → Speech → Text to Speech. Remote speech requests follow the application-wide proxy setting.
...
Rule Management has one search field and two ordered sections: user-authored rules first, then the current subscription rules. Both sections support search; subscription rules are editable as normalized Mihomo source lines, and a subscription refresh replaces those edits while leaving user rules intact. User rules support Mihomo domain, wildcard, regular-expression, IP/CIDR, port, process, network, and logical matcher types with Direct or Proxy actions. Rules that require external GeoSite, GeoIP, or ASN data, or remote `rule-providers`, are excluded from the self-contained runtime and counted in the subscription summary.
