...
本页对应 `ToolPermissionSystem` 的调用权限，不替代 Android 系统授权。统一 R0–R3 风险分级
仍是 [目标设计](../contracts/extensions_workspace.md#应用内-ai-动作授权)，不能假定设为 `ALLOW`
的工具会在每个高影响动作前再确认。具体工具的独立确认与限制以其实现为准。
