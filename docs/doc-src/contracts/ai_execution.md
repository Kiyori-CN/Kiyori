...
- 对话栏的统计图标保留上下文进度圆环，打开统一模态底部面板。可切换“对话累计 / 最近请求”，展示提示词缓存、上下文、速度、延迟、用量和覆盖；最近请求展示实际 Provider 模型标识。复制摘要仅包含模型标识、统计数值及口径，不复制对话正文或凭据。
...
- 流式期间使用已有 token 回调并标为估算；完成后按 `ProviderUsageSnapshot` 校正输出量和来源。非流式、单输出块、零时长、无输出、无 usage、流修订回滚均不伪造有效速度。完整调用即使没有速度，也可保留其用量和客户端计时。首次输出等待、请求总耗时从调用开始计算，包含准备、网络、代理和服务端等待；首次内容可能是思考或工具数据，不冒充纯文本 TTFT、网络 RTT 或纯服务端推理耗时。取消/失败清理当前样本，下一模型请求重新计量。UI 只在面板打开时订阅速度，避免高频分片重组整个聊天栏。
- `GenerationSpeedMonitor` 只允许当前请求发布，旧请求的迟到更新/清理无权覆盖新请求。统计委托按 chat 隔离内存样本；会话切换后使用对应样本，不持久化或从历史累计数推算速度。
- 累计用量继续使用已入账的 `ProviderUsageAggregate`，正在执行的回合稍后计入。总量为输入加输出，缓存是输入细分，推理是输出细分，均不重复相加。最近请求按字段区分推理/缓存写入未提供与明确 0；历史存储没有这两项的字段覆盖，零值显示未提供并注明限制，不反推历史事实。
- 没有任何供应商用量或缓存指标时，相应字段显示未提供。完整供应商用量覆盖要求输入和输出均有报告；部分快照不将本地估算补成供应商值，最近请求仍显示已知输入/输出。缓存读取率按 token 加权：有效缓存读取 / 有效缓存指标请求的输入量；分母与缺少缓存指标的输入分别展示。异常缓存分桶不进入分子，输入总数保留在普通/未分类输入桶；历史分子超出分母时显示未提供。计数饱和相加，聚合按 hop 去重，新 hop 增量入账，替换才重新折叠。
- OpenAI/DeepSeek、Anthropic、Gemini、ToolPkg 各自按协议解释总量与分桶，拒绝负数、非整数、错类型和越界计数作为有效缓存证据。DeepSeek Chat 的命中与未命中之和必须匹配总输入；两者完整时可恢复缺失总输入。Anthropic 原生输入不含缓存读写，缓存创建只求和协议定义的 5m/1h 桶。Gemini 输出包含候选与思考，尾块更新总输入时重算未缓存桶。
- 只读统计不使用禁用菜单项。底部面板复用统一滚动、模态隔离、拖动、关闭和 Back；数字不截断，大字体或窄窗口的标签和值上下排列；入口提供上下文百分比无障碍描述。

面板请求计数沿用已关闭并入账的 Provider hop，不等于 HTTP attempt 总数，也不证明未返回用量的
失败请求没有产生费用。响应尚未确认、取消、断流与副作用身份仍以对话审计及执行账本为准；
不能从本面板反推出远端未执行，或据此授权重新发送。
...
- 纯思考或工具调用截断时的宿主警告属于普通反馈，只有真实执行结果才使用 `TOOL_RESULT`。
  截断调用组全部作废后，宿主追加带该调用组 SHA-256 的 `kiyori_invalidated_tool_round` meta；
  唯一 replay projector 仅移除字节匹配且没有任何工具结果的整组调用，保留后续回答和有效事务。
  普通警告、哈希不符或已经部分执行的组不能获得此豁免。实时正文与审计保留作废调用及标记，
  最终持久化正文使用同一个安全投影，不生成虚假的工具结果。
- 手动及自动总结的历史清理、工具裁剪和摘要拼装在后台 dispatcher 执行。总结 Provider 请求
  携带独立本地执行身份并关联原聊天；非取消失败记录 `COMPACTION_FAILED`，保留原异常与原历史，
  发送触发的异步总结也必须展示失败。传输提交未知仍遵守单次提交合同，不自动重发或换模型。
...
- GPT-5.6/Astra Responses 的官方和兼容端点均可使用标准 Prompt Cache key；DeepSeek 继续自动缓存，不发送该键。key 使用 profile revision、精确模型、稳定 system/developer 前缀和 canonical 排序的
...
- OpenAI Chat 默认请求 `stream_options.include_usage=true`，保留显式关闭；Responses 不注入 Chat 参数。
  官方 GPT-5.6/Astra Responses 在初始 developer 前缀末尾添加一个明确缓存断点，保留默认 implicit 的历史边界；已有显式缓存模式/断点保持调用方策略。兼容端点不自动获得断点、TTL、background、store 或 encrypted reasoning 等官方扩展。
- DeepSeek Responses 把 `developer` 视为用户消息，因此系统提示必须保留 `system` 角色；普通 OpenAI Responses 继续映射为 `developer`。DeepSeek 明文 reasoning 与工具结果保持原有重放和身份闭合，不引入未知提交重发。
