...
- “更多功能”最底部的独立“使用引导”分组提供“重新查看首次引导”，与开源和法律条目分组隔离。设置重看使用独立全屏模态窗口隔离触摸与焦点；每次新进入从第一页开始，关闭时保留设置来源。重看不改写首启完成与步骤偏好；查看协议正文后保留原阅读位置。
- 重看 Pager 只属于当前组合会话，不恢复旧页码；首启继续保存恢复进度。普通按钮只允许当前页发起相邻切换，只有明确点击“跳过介绍”可向前跨页到第五页；滚动期间拒绝导航，按钮外观不随滚动切换禁用配色。固定栏按钮的拖动、折返和多指手势不能形成点击：Initial pass 在原生 clickable 收到抬起前消费无效 up，Final pass 结束也不恢复已取消的资格；原生键盘和无障碍点击不依赖旧触摸状态。
- 前四页使用统一眉题（第一页为 `Kiyori`）、紧凑三屏导航示意与能力卡，每页四项。说明以两句表达能力和真实入口；可用内容宽度小于 `320dp` 或字体缩放不小于 `1.3` 时单列，其他情况双列。文字自然换行，不压缩字号或隐藏说明；模型、语音和终端标明需先配置。内容宽度不小于 `600dp` 时左右分栏，短屏/大字体省去示意图，正文仍可滚动。卡片仅说明，不承担导航；页码播报由唯一顶栏持有。
- 系统授权只处理用户所选条目。返回后刷新权限事实，继续下一项与完成引导均需明确操作；旧代际回调不能更改新队列，停止后保留系统已授予的权限。
- 首启与设置共用 `KiyoriPermissionId` 目录、分组、metadata、快照与动作。首启摘要仅强调所选数量和选择权，逐项状态完整呈现，正文与底栏按实际高度分配空间；系统权限弹窗未返回时不允许翻页。当前目录包含 23 项；逐项选择只包含当前可处理且未完成条目，不授权也可以进入应用。
...
所有底部抽屉复用 `KiyoriDraggableBottomDrawer` 或相同的菜单表面 token：拉柄与正文同为当前主题 `surface`，
统一圆角与遮罩，不添加拉柄分隔阴影或抽屉外阴影。长标题使用可增长行高，关闭动作保留空间；文件表单固定标题、滚动正文。
系统栏和键盘由共享宿主消费一次。拖动同步累计位移，快速反向甩动以松手速度决定方向，关闭动画后再卸载业务内容。
