...
书签、历史、下载、资源嗅探、网络日志、诊断日志与扩展的操作固定在标题同一行最右侧；下载管理菜单
紧邻标题文字。历史、嗅探、网络、诊断及扩展头部不再显示总数，批量选择和分类计数保留原义。
扩展的加号菜单复用原有新增与导入入口。
历史、下载、嗅探及网络日志共用紧凑描边按钮，视觉最小高度 32 dp，删除/清理使用语义红色，
空列表禁用；保留 Material 最小交互面积，大字体允许内容增高。窄窗口优先保留操作，标题单行
省略，操作超出可用宽度时横向可达。其他多操作子页沿用原分行布局，安全区仍由公共宿主持有。
资源嗅探清空只移除当前 WebSession 已捕获候选，不清理网络/诊断日志、不停止播放或下载；
后续请求继续由同一会话采集。候选消失时清除失效筛选及旧候选弹窗。

网页工具仅使用当前宿主已有 WebView：阅读模式与网页朗读共用同一次正文提取，取 article/[role=main]/main/body
中可见文字最多的一个作为正文根，只收集最内层块级元素并按标题、段落、列表、引用、代码保留结构；
导航、页眉页脚、表单控件与隐藏节点排除，限 120000 字符并标明截断；结构化收集不到块时按根节点
innerText 换行切分。脚本被页面拒绝与页面确实没有正文分别提示，均可原地重新提取。
阅读模式进入固定白底黑字的整页阅读界面（不跟随应用主题），提供字号档位、阅读进度、正文选择复制
与朗读开关，系统 Back 退出并停止朗读；朗读由系统 TTS 分段处理，退出即停止并释放引擎。
导航或文档变化使阅读页与异步提取结果失效。
