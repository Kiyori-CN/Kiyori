...
- 新建下载使用同一弹窗编辑完整链接，剪贴板仅在用户点击粘贴时读取；文件命名可选，扩展名不执行格式转换。提交前校验完整 HTTP(S) 地址、路径分隔/控制字符及组合文件名 UTF-8 长度，合法链接仅裁剪首尾空白，签名参数不重写。
- 手动文件名从解析后的 URL 末段路径识别并解码；没有文件路径时使用 `download`，自动名称中的非法文件字符替换为下划线。自定义名称与扩展名在同一命名策略组合，完整复合扩展名不重复追加。
- 表单标题与主操作固定、内容可滚动；失败保留输入，接受前锁定重复提交。内部任务创建后进入下载中列表，系统任务继续交给 Android 下载管理，不在内部列表伪造进度。
