...
Debugger 命令入口统一使用 `/system/bin/sh -c` 解释完整命令，不自行拆分 argv 或修改引号内文本；
环境变量赋值不能作为程序名执行。文件管理器隐藏项与列表大小由 `FileManagerPreferences` 持有，设置页与双栏共用；默认排序只在新会话初始化，当前排序与过滤按栏持有。
搜索结果与文件任务记录存入不参与备份的私有历史目录，结果未知不重放；详见文件管理器设计。
