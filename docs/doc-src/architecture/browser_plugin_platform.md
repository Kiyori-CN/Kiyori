...
- Android WebView 不支持 userscript runtime 时，总授权开关、工作台菜单和授权横幅均不可请求开启；
  “不支持”与“未授权”保持独立状态
- 插件中心“本页 / 已安装”标签数量按当前实际渲染的 provider 卡片计数，搜索后不会继续显示过滤前数量
- 脚本详情的源码标签可导出当前 active revision 为 `.user.js` 到
  `Download/Kiyori/exports`；导出不修改正式 revision、registry 或脚本启用状态
- 删除仍清理脚本 registry、active source/revision、草稿、值与日志。因为这些数据使用数值
  `scriptId` 绑定，删除 registry 后“保留数据”会形成无法重新关联的孤儿数据，所以当前不提供该伪选项
- 单项删除失败会写入应用日志并显示错误；批量删除逐项隔离异常，一个项目失败不会阻止其余选择项继续处理
...
- 新安装默认开启；关闭后安装与更新仍可进行，但已启用脚本显示“需要授权”且不会注入页面
...
- 权限和 `@connect`
- `@require` 与 `@resource`
- 源码与当前 active revision 的 `.user.js` 导出
- 日志
- 版本

删除确认必须明确说明源码、修订、草稿、值和日志会一起清理。当前数据身份按数值 `scriptId` 绑定，
在没有稳定脚本身份和显式重新关联协议前，不提供“删除脚本但保留数据”选项。
