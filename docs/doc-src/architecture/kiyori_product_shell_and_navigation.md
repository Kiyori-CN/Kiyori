...
- [Back 契约](#back-契约)
- [全屏网页搜索](#全屏网页搜索)
- [AI 首页与模态 AI 左抽屉](#ai-首页与模态-ai-左抽屉)
- [负一屏](#负一屏)
- [文件管理与设置首页合同](#文件管理与设置首页合同)
- [设置所有权](#设置所有权)
- [自适应布局](#自适应布局)
- [AI 能力合同](#ai-能力合同)
- [浏览器 source-port](#浏览器-source-port)
- [上游同步边界](#上游同步边界)
- [参考实现](#参考实现)
- [Kiyori 首次启动合同](#kiyori-首次启动合同)
- [待决策](#待决策)
...
“设置 - 更多功能 - 权限管理”是 Kiyori Settings 自有的设备能力页面。它和首次启动第六页
共同消费 `KiyoriPermissionId`、`KiyoriPermissionSnapshot`、共享 metadata、状态语义和授权动作，
不保存第二份授权结果。Settings 页面按“应用权限 / 系统访问 / 高级设备能力”分组显示全部 23 项，
提供总览、原位刷新、逐项处理和按真实顺序处理全部待授权项；首次启动复用相同三组顺序、说明、
状态摘要与动作，支持逐项选择和全选当前 snapshot 中可处理且未完成的项目。全选只改变
待处理集合，不代表已授权；处理中禁止改变选择，系统或能力提供者继续返回真实结果。
...
为两个独立文档；正文版本由 `AgreementPreferences.CURRENT_AGREEMENT_VERSION` 声明。首启接受条件只由一个明确同意复选框控制，
不要求先打开文档或滚动到末尾。
