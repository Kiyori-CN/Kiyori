...
- `KiyoriSemanticTone` 为设置、应用壳、AI 支持页和浏览器内容工具提供蓝、绿、紫、橙、红、青和粉色的固定浅深主题图标容器；设置页面继续由 `KiyoriSettingsTheme` 提供亮色灰白/白卡和暗色近黑/分层深灰表面
...
- App Shell 底栏未选中态保留原空心 Vector；选中态不绘制胶囊、圆形背景、蓝色描边或可见圆形涟漪，而是使用浅色 `#C48A00`、深色 `#FFD166` 的封闭填充层，并以当前页面背景色重绘原图标线条和内部细节
- AI 首页顶栏浏览器、终端、工作区动作固定使用蓝、青、紫语义色；终端与工作区仅在活动时增加低饱和同色容器
- 软件首页天气图标按天气语义着色，晴天与底栏选中态共享暖黄色，权限和数据异常使用红色或橙色
...
- 设置首页保留固定复刻布局和 `4/4/4/4` 密度，第一组改为“账号与连接 / AI 助手 / 语音服务 / 小程序管理”；账号、AI、语音、浏览器、播放器、下载器、界面和数据入口连接现有 owner，其余入口没有导航副作用
- 小程序管理只为底部第三个小程序产品域保留空入口，不连接 AI 包管理、ToolPkg、脚本包或插件市场
