...
- Kiyori 面向 Android 的人机任务协作，浏览器与内容处理是已实现的核心工作环境
- AGI 是 README 中记录的长期研发方向，不代表当前能力或现场验收结论
...
2026-09-12 对齐已有 README 愿景与开发语义：早期“全能型浏览器”定位解释应用壳独立的
起点，当前目标继续扩展到终端、文件、媒体和工具协作。此处消除文档漂移，不新增运行时、
改变稳定接口或将后续设想记为已实现。

- 后续上游同步需要区分 AI 内部能力和 Operit 应用壳变更
- 新页面必须明确属于 Kiyori 系统、内容域或 AI 子模块
- 全局语言、产品级主题和其他跨域设置迁入 Kiyori 系统设置
- 用户可见的“AI 助手”设置只保留 AI 专属配置，并由 Kiyori 设置首页进入；模态 AI 抽屉底部的通用“设置”进入来源保持型设置首页，不再把 AI 助手设置作为独立抽屉一级入口
- 设置首页的“小程序管理”只为底部小程序产品域保留，不承接 Operit AI 的包管理、ToolPkg、脚本包或插件市场
