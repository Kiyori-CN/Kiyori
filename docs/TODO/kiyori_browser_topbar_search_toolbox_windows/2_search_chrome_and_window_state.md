...
- 第一行固定为返回、输入卡片和搜索三个槽位，沿用浏览器顶栏的 `8dp / 6dp / 40dp / 42dp`
  几何；输入卡片顶部、返回和搜索按钮的绝对位置不随输入行数改变
- 输入卡片包含当前搜索引擎、`搜索或输入网址` 占位和条件清除按钮；提交搜索按钮位于输入卡片外
- 输入卡片按真实布局行数从一行基准向下增高，最多显示三行，超出后在三行视口内纵向滚动；引擎按钮、
  清除按钮、占位符和输入文本始终在当前卡高内垂直居中
- 引擎面板从输入卡片下方展开，当前引擎高亮；点击面板外只关闭引擎面板
- 当前 URL 有效时显示当前网址卡片，提供打开、复制和回填输入框三个动作
- 搜索记录使用自适应标签，单条可打开或删除，标题区域提供一键清空；无痕 Profile 动作位于标题右侧
- 打开搜索页、切换引擎、删除记录和清空记录都不触碰活动 WebView

浏览器结果页的搜索引擎切换条由搜索 recovery 与当前结果 URL 的精确匹配决定，并额外受“网页浏览器 →
搜索引擎切换条”持久化开关控制。离开结果页（包括回到自定义主页或普通子页）会同时清除投影 query 与
可见状态；关闭开关只改变呈现，不复制 recovery 状态。浏览器“滑屏前进后退”开启后支持左右边缘起始
手势，也支持从网页中部滑向对应边缘的单指水平手势；未到边缘的普通横向滑动保持网页所有权，所有手势
继续复用唯一 WebView 历史 Back/Forward owner。
...
空输入不导航，也不改变当前页面。打开记录直接导航其已持久化的 `targetUrl`。网页 Cookie 插件子页进入
时只自动读取一次当前 HTTP(S) 页面 Cookie；后续重读必须由用户点击刷新触发。
