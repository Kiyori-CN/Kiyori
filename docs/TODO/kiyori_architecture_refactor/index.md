...
工作区均干净。范围是根目录、构建职责、工具入口、命名规范、架构检查和开发文档。
本轮完成本地实施、验证、提交和推送；不操作设备，设备验收仍按各领域状态单独记录。
详细盘点、取舍与验收见 [仓库规范化实施记录](23_repository_structure_and_build_logic.md)。

| 阶段 | 交付物与退出条件 | 状态 |
| --- | --- | --- |
| R-01 | 根目录、九个 Android 模块、工具、文档与兼容边界盘点；确认实际问题 | DONE；491 份文档检查通过，发现 7 项首启旧架构断言、构建任务混杂和工具记录漂移 |
| R-02 | 提取构建任务，保持任务名、输入输出、Variant API 与打包断言；补行为验证 | DONE；buildSrc 行为测试通过，Debug 构建成功 |
| R-03 | 工具目录命名和入口归位；清理确定的跟踪缓存；修复失效架构检查 | DONE；工具迁移、缓存清理和 ARCH009/ARCH010 扫描范围已闭合 |
| R-04 | 正式文档、目录与 CI 同步；相关检查通过并核验 Debug APK | DONE；文档 495 文件、Python 316/316、架构/正式准备和 APK 审计通过 |

本轮工程整理不关闭下方 v4 全领域运行时重构或设备验收。各自状态继续由实际证据裁定。
...
目录、`SKILL.md` 格式、导入协议、可见性偏好和系统提示词内容。`SkillManagerLifecycleContractTest` 定向
