...
- `DONE LOCALLY`：网络代理主页面已收敛为摘要入口；模块连接和订阅策略组/节点分别进入页面内二级
  视图。策略组视图按当前查看的订阅提供横向可滚动分组标签；`select` 组中点击节点行直接切换当前项目，
  行尾按钮执行单节点测速，底部命令执行整组测速，全部继续由同一 `KiyoriNetworkProxyManager` 执行。
...
4. **模块连接**：主页面只显示七个模块的生效模式摘要；点击后进入独立二级视图，逐模块选择“跟随默认 / 直连 / 代理”。
   页面解释覆盖优先级：应用内总开关 -> 模块覆盖 -> 传统脚本包覆盖，并明确“直连”仍可能经过系统 VPN。
5. **策略组与节点**：主页面只显示当前订阅和最终节点摘要；点击后进入独立二级视图。视图顶部选择订阅，
   标题行右侧固定显示“测速 / 布局 / 排序”三个操作：测速直接测试当前横向标签对应的整组，布局菜单提供
   单列、双列、多列，排序菜单提供默认、名称、延迟。订阅内所有策略组使用横向可滚动标签切换，标签显示
   真实分组名，保留组使用“默认代理”。标签下面单独一行是当前分组节点搜索框，只过滤显示结果，不改变
   运行配置。组详情显示类型、项目数量和当前项目；组内节点显示类型、最近延迟、成功/超时/失败状态。
   `select` 组点击节点行直接切换，节点行尾测速按钮只测速该节点，自动策略组明确显示由 Mihomo 决定，
   不提供伪装的手动切换；底部仍保留整组测速命令。provider 动态节点由 Controller 探测后进入同一清单。
6. **脚本规则**：进入可搜索底部抽屉，列出已启用且可执行的传统 JsEngine 包，逐脚本选择“跟随脚本模块 / 直连 / 代理”；没有脚本时明确提示数据来源，并保留手动包名规则。ToolPkg 不伪装成传统脚本；ToolPkg 宿主 HTTP 归入 `AI_TOOLS`。
