...
- 权限页与设置内“权限管理”复用同一 23 项目录、三组结构、真实 snapshot、状态口径和授权动作
- 首次启动支持逐项选择和全选当前可处理项；选择来自共享 snapshot，不把已完成、使用时确认或不适用项纳入授权队列
...
顶部显示 Kiyori、当前步骤文案、数字进度与六段进度指示，并消费状态栏、显示缺口、横向边缘与导航栏的
...
- 全选仅使用 `permissionSnapshot.selectable`，授权中禁止改变选择；真实系统授权仍逐项或按 runtime 权限集合执行
...
以下是该日期的实施与验证记录；其中 21 项目录、取消全选和旧顶部布局已由后续权限扩展及
首启重设计替代。当前共享状态、全选边界和进度来源见上方当前流程与
[应用壳契约](../../doc-src/contracts/product_shell.md)。2026-09-06 的 ARCH037 整理保留
共享权限、处理中锁定、协议与启动单一 owner 检查，并改为当前步骤和可处理项选择的正反例。
