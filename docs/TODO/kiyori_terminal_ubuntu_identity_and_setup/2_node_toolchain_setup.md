...
- 就绪命令要求 Node.js 主版本至少为 24，并实际执行 `pnpm --version` 和 `tsc --version`；三项全部成功时才输出唯一完成 marker
- 安装页和 Kiyori 父应用都使用同一命令与结果解析，删除两套易漂移的判定

## 边界

- 可见终端使用交互式 login Bash，AI 后台命令使用 `--noprofile --norc`；两者都继承终端固定的 npm 全局 bin，因此不修改用户 shell 配置
- 不改变 NodeSource、npm 镜像或已选的 Node.js 24 主版本
- 不增加失败后继续执行、替代包管理器或静默忽略错误的逻辑

## 状态 [DONE]

安装页与父应用已共用 `TerminalEnvironmentContract`。npm 在同一全局 bin 中安装 pnpm 与 TypeScript；就绪检查要求 Node.js 主版本至少为 24，且 `pnpm`、`tsc` 均能执行。
