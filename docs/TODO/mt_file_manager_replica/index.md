...
[浏览器补丁整合](../kiyori_browser_product_completion/index.md#2026-09-13-浏览器与文件首页补丁整合)。
首页减少图标网格、分组标题与存储项间的纵向留白，保留存储行至少 48 dp 触控面积；末尾净空
随实际底栏尺寸与系统导航栏 Insets 调整。普通文件“打开方式”新增内置浏览器，保留唯一文件会话与 Shell 来源。
...
本轮授权实现、验证、构建、提交及推送；不安装或操作设备，不修改 terminal，不新增文件后端。

1. 统一“文件管理器”折叠标题、分组卡片、设置行和选择抽屉，复用下载器与浏览器设置组件。
2. 按七组整理生效范围、当前值和确认文案；启动路径提供快捷填入且保存前校验。
3. 隐藏清单提供搜索、逐项及确认后批量取消隐藏，隐藏入口保留可辨识身份。
4. 历史数量来自原历史所有者；修复清理失败的内存删除标记，验证正在运行的任务保留。
5. 使用类型化面板状态和集中重置函数，检查主题、长内容、无障碍与异步操作生命周期。
6. 定向回归、文档检查、Debug APK 核验后审计精确文件清单，提交候选树并核对远端 main。

依赖为现有 `FileManagerPreferences`、`FileManagerHistoryStore` 与设置组件；保留 Shell 会话所有权。
风险集中在重置保留字段、失败写盘和长内容布局；以本轮提交差异回滚，不迁移或修改实际文件。
设备的深浅色、大字体、键盘、返回和真实存储验收保持 `verification_pending`。
...
本轮授权本地实现、验证、提交与推送；不操作设备，不修改 terminal 或新增文件后端。

1. 统一 AI 与文件管理四个顶栏按钮几何及中性图标；刷新只保留图形反馈，双栏增加贴边可拖动箭头柄。
2. 统一底部抽屉的表面、拉柄、标题与可滚动内容，检查键盘、安全区、长内容和关闭行为。
3. 完善手动隐藏清单与系统隐藏开关、目录大小、工具箱顺序和离开文件页后的悬浮恢复。
4. 定向验证隐藏持久化、导航、刷新/滚动及相关文件管理行为，串行构建 Debug APK。
5. 审查最终允许清单和敏感内容，提交 main、检查候选树，推送并核验远端 ref。

复用 FileManagerViewModel、FileManagerPreferences、Shell 和共享抽屉所有者。
风险集中在触摸冲突、迟到目录结果、隐藏身份和离页生命周期；按本轮提交回滚，手动隐藏不改动文件本体。
真实触摸、主题、大字体、软键盘与存储现场验收保持 `verification_pending`。

本轮证据（2026-09-12，Asia/Shanghai）：

- 四动作槽对齐、AI 中性图标、无文字刷新与阈值触感、可拖动上下箭头柄、棕金色文件恢复球及工具箱顺序已接通。
  共享抽屉统一表面与零阴影，修复逐事件协程竞争和反向甩动方向；文件表单固定标题、正文滚动。
- 手动隐藏按环境/路径隔离并同步双栏，清除隐藏选择，可编辑/取消隐藏；可见本地目录经既有文件检查统计内容大小，
  共用限量缓存、超时和代际保护，不把失败显示为零。设置返回不会复活已关闭的文件会话，写入中不允许关闭恢复球释放会话。
- 最终定向 JVM：27 套、272 项，0 失败/错误/跳过；覆盖文件管理、偏好持久化、Shell、共享抽屉、浏览器布局与主题。
  首轮新增的设置关闭后返回测试失败，返回条件修复后最终命令 `BUILD SUCCESSFUL`（5m09s）。
- 文档检查 521 文件无问题，正式准备检查通过；两份 Shell 哈希按实际差异审阅更新后，完整架构检查 `phase=m03` 通过。
- 串行 `:app:assembleDebug --no-daemon --console=plain`（6m02s）成功；唯一 Launcher、脚本代理和播放器打包检查通过。
  APK 生成于 `18:30:39 +08:00`，`488291759` 字节，`com.kiyori / 45 / 0.1.0 / arm64-v8a`；V2 单签名与 16 KiB zipalign 通过。
  SHA-256：`391635790530a9fe96cf4b42615b0b16f12836ccc4a20bf2a5757b9ed8aba050`。
- 交付允许清单为 30 个源码、测试、文档与架构记录；terminal 固定 gitlink 不变，APK/日志/私有日记不进入 Git。
  本地及提交交付与设备验收分别记录；本轮未安装设备、运行 Release 或宣称远端 CI 通过。
