...
## 2026-09-12 顶栏、分列浏览与持久历史

状态：实现与本地验证完成，设备 `verification_pending`。基线 `main / ada31d38613cfaad5302119489b635f514cd6356`，初始工作区干净。
用户已确认方案并授权验证后提交推送；不安装设备、不修改 terminal、不更换文件后端。

1. 浏览状态：每列独立名称/格式/大小/修改时间过滤、确认应用、独立排序、完整交换与请求代际隔离。
2. 历史：保存最近 20 次搜索与结果快照、最多 100 条文件任务；应用私有原子存储，终态不明不自动重放。
3. 交互：四个顶栏按钮、三个既有底部抽屉、分列弹性下拉刷新、工具箱与进行中/最近任务面板。
4. 验证：过滤边界、窗口互换与迟到请求、历史重建及失败路径、文件管理回归、文档与架构检查，串行 Debug APK。
5. 交付：审查允许文件、敏感数据与子模块，提交 main、检查候选树、推送并核验远端 ref。

依赖原 FileManagerViewModel、目录快照、文件工具与主题，不新增文件执行所有者或依赖。
风险集中在手势冲突、目录/搜索异步发布、历史持久化与批量操作；回滚按本轮提交，保留原文件与回收数据。
真实触摸、字体/主题和设备存储验收单列 `verification_pending`。

本轮证据（2026-09-12，Asia/Shanghai）：

- 顶栏四入口与既有底部抽屉接通；确认过滤、分列排序与完整交换复用原窗格所有者。
  同路径交换额外校验呈现代际，拒绝旧滚动监听写回；手动刷新与横向滑选保持不同手势方向。
- 搜索保存条件与结果快照，打开旧结果先核对存在性；删除记录不被迟到结果恢复。
  文件任务先记录进行中、再更新终态，未知结果不重放；旧单结果回看状态已替换为统一历史。
- 最终定向 JVM：19 套、164 项，0 失败/错误/跳过，覆盖目录/搜索/文件任务生命周期、过滤边界、
  分列排序与交换、历史原子保存/重建/裁切/损坏保护、默认偏好和设置来源导航。
  首轮发现点文件扩展名误匹配及过期源码断言，均已修复；最终命令 4m01s 成功。
- 文档检查 521 文件无问题，正式准备检查与完整架构检查 `phase=m03` 通过。
- `:app:assembleDebug --no-daemon --console=plain` 1m05s 成功，唯一 Launcher、脚本代理与播放器打包校验通过。
  Debug APK：`app/build/outputs/apk/debug/app-debug.apk`，生成于 `17:22:04 +08:00`，488291759 字节，
  `com.kiyori / 45 / 0.1.0 / arm64-v8a`；新过滤、刷新、任务抽屉与历史类已核对进入 DEX。
  SHA-256：`c42a7c2e9417134f88a7116c8ff618a26f0ec745a130dcecf47172f898f035da`。
  APK v2 单签名与 16 KiB zipalign 检查通过。
- 32 个文件进入精确交付清单，未包含 APK、日志、私有日记或 work 检查点；terminal 不变。
  未安装设备、运行 Release 或宣称远端 CI 通过；弹性手感、键盘/大字体、真实存储和进程重启体验仍需现场验收。

## 2026-09-12 目录执行回归与设置页

状态：实现与本地验证完成，设备 `verification_pending`。基线 `main / e6549a496`，初始工作区干净。

- 截图显示 Shizuku 授权与执行器均可用，但 `LC_ALL=C` 被当作程序启动，返回 `error=2`。
  根因是 Debugger Shell 入口按少数操作符猜测命令语法；改为统一 `/system/bin/sh -c` 执行完整原文，
  清理旧参数拆分和字符串改写，沿用原权限路由、进程生命周期和并发输出读取。
- 目录错误页保留简短说明、重试和详情入口；完整错误可滚动、选择和复制，避免双栏被诊断长文本撑满。
- 设置首页的文件管理器接通专属子页，持久化隐藏项、排序方式/方向、列表大小；与双栏快捷操作共用
  `FileManagerPreferences`，设置往返保留会话，权限子页遵循来源返回，回收站规则明确可读。
- 不改变全局权限选择，不安装设备、不增加自动降级；回滚以本轮差异为边界，既有回收数据不变。
- 验证计划：模拟 Shizuku 进程的真实 argv、退出码，偏好保存/重建与双栏同步，文件管理器及设置导航回归，
  文档与架构检查，串行 Debug APK 构建；手机目录读取与视觉仍需现场验收。

本轮证据（2026-09-12，Asia/Shanghai）：

- 定向 JVM 回归 23 套、279 项，0 失败/错误/跳过；覆盖实际 Shizuku `newProcess` 参数、非零退出码、
  持久化重建、双栏同步及设置来源返回。首轮唯一失败为“文件管理器必须为空入口”的旧断言，已按新行为更新。
- 文档检查 521 文件无问题、正式准备与完整架构检查（`phase=m03`）通过；架构记录精确增加偏好存储及设置接线。
- 最终 `:app:assembleDebug --no-daemon --console=plain` 1m42s 成功，唯一 Launcher、脚本代理与播放器打包校验通过。
  APK 为 `app/build/outputs/apk/debug/app-debug.apk`，生成于 `15:23:39 +08:00`，488291759 字节，
  `com.kiyori / 45 / 0.1.0 / arm64-v8a`；新设置页、偏好存储和错误详情类已实际进入 DEX。
  SHA-256：`e55db28c41fea96a8e9abb26d11d70744eff20a6ccd5d72599b050376cad1382`。
- 29 个文件进入精确交付清单，APK、日志、日记和 work 检查点不进入 Git，terminal 未修改。
  未安装设备或运行 Release；手机共享目录读取、Shizuku 受限路径、设置持久化及浅深色/大字体效果仍待复测。
