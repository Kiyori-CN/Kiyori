...
- 自动门禁：M-02 至 M-05 的当前可重现 checkpoint 为 `main@6b6493a0`。完整 architecture 为 `phase=m03`、`errors=[]`，CI Python 为 `166/166`，Android JVM 报告为 `137` 个 suite、`822` 个测试、零失败、零错误、零跳过；formal readiness、fresh clone、working-tree Markdown 和 `git diff --check` 均通过。QD-01 至 QD-03 已把 fresh full lint 从 `27 errors / 287 warnings / 3 hints` 收敛到 `0 errors / 58 warnings / 1 hint`；Gradle 控制台对同一报告显示 `58 warnings`，差异为不计入 actionable 摘要的 `LintBaseline` 状态提示。current-only 债务和完整 Kotlin 重编译暴露的历史 compiler 警报继续按 [Stage 4 前质量债务与开发就绪精确清单](../kiyori_architecture_refactor/21_quality_debt_and_stage4_readiness_manifest.md) 收口。现有 baseline 为 `5787` 条，SHA-256 `80691E34E07299ABAA58C5F19DBDB27FFE5DB65AF03DEA2514B8A2AC0CC4F9E4`；结构化交集只删除 4 条已失效记录，没有吸收 58 条 current-only 警报或新增 suppress
