...
- Debug 构建：包含 FFmpegKit `8.1.7-kiyori-n9.0.1-r6` / OpenH264 `v2.6.0` 的规定 `:app:assembleDebug --no-daemon --console=plain` 重新验证为 `BUILD SUCCESSFUL in 1m49s`，`232` 个任务中 `32` 个 executed、`200` 个 up-to-date；唯一 launcher、player native input 与 runtime packaging 门禁通过。APK 路径为 `app/build/outputs/apk/debug/app-debug.apk`，写入时间 `2026-08-18 03:34:17 +08:00`，`494168730` bytes，SHA-256 `5C12152E9F180B1454AFAF8FD72F370840B409E8703902E9BEDE6A434250285E`；包名/版本/SDK 为 `com.kiyori / 45 / 0.1.0 / 26 / 34 / 37`，Android Debug V2 单 signer 与 16 KB zipalign 通过
