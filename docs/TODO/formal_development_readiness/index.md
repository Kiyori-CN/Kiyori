...
- Debug 构建：包含 FFmpegKit `8.1.7-kiyori-n9.0.1-r6` / OpenH264 `v2.6.0` 的规定 `:app:assembleDebug --no-daemon --console=plain` 于 2026-08-19 第二次设备边界修正后重新验证为 `BUILD SUCCESSFUL in 37s`，`232` 个任务中 `22` 个 executed、`210` 个 up-to-date；唯一 launcher、player native input 与 runtime packaging 门禁通过。APK 路径为 `app/build/outputs/apk/debug/app-debug.apk`，写入时间 `2026-08-19 14:01:12 +08:00`，`467107608` bytes，SHA-256 `12EF31588AA26E0B2CD0863958A9FAB97EE6FD6B74642A5813F180543F8E43C1`；包名/版本/SDK 为 `com.kiyori / 45 / 0.1.0 / 26 / 34 / 37`，Android Debug V2 单 signer 与 16 KB zipalign 通过
