...
3. 独立高频操作按钮继续保持至少 40dp 触摸目标；密集的横向筛选/切换条允许使用
   `28-36dp` 的紧凑高度，以释放内容空间。图标按钮提供 `contentDescription`，仅图标的陌生动作
   提供 tooltip 或可读语义；文本操作使用文字按钮，撤销、关闭、返回、搜索等使用熟悉图标。
...
- 本轮按用户反馈恢复紧凑横向选项条密度：浏览器嗅探徽标恢复 `28dp`，搜索引擎快捷芯片/关闭按钮
  恢复 `28/22dp`，AI 对话统计球与历史/画中画控件恢复 `32dp`，AI 次栏外层保持 `2dp` 上下留白。
  历史、网络日志、插件、脚本工作台、资源格式、下载状态、播放器日志、市场、ToolPkg 环境变量、
  网络代理、文件工作区、文件管理器和设置页的横向筛选/切换条统一收紧；通用筛选芯片采用
  `36dp`，资源/下载状态采用 `32dp`，外围垂直留白由 `8-9dp` 收至 `4-5dp`；包含图标与文字的
  设置标签保留 `56dp` 的可读高度，避免裁切。独立高频动作按钮、导航状态、业务逻辑和状态 owner
  不变。
