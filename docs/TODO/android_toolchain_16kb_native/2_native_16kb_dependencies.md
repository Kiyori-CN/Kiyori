...
最终 Debug APK：`app/build/outputs/apk/debug/app-debug.apk`，时间 `2026-07-23 14:19:31 +08:00`，大小 `413888093` 字节，SHA-256 `C46360981D04B975EF738E2A5D9932CAE121C125C16D662FA73508D86ABBD3EA`。

- `zipalign -c -P 16 -v 4`：通过
- arm64 `.so`：43 个文件，其中 42 个可识别 ELF、33 个 `LOAD >= 0x4000`、9 个未对齐 ELF、1 个非 ELF
...
Lint 最终报告为 46 warnings + 2 hints，其中 `Aligned16KB=3`，均来自 ffmpeg AAR。未被源码使用的 TensorFlow Lite 2.10 已移除，其 x86_64 告警与 duplicate namespace 构建错误同时消失。
...
- AGP 9.3.0、Gradle 9.5.0、Kotlin 2.3.21 与 Build Tools 36.0.0 已完成迁移；CI 实际执行仍取决于重新启用仓库级 GitHub Actions
