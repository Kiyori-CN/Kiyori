...
本阶段只接受一套 FFmpeg/libmpv 动态链接栈：

| owner | 固定来源 | 输入 SHA-256 | 进入 arm64 APK 的库 |
| --- | --- | --- | --- |
| FFmpeg 与 C++ runtime | `dev.ffmpegkit-maintained:ffmpeg-kit-full:8.1.7`，源码 tag `v8.1.7` / `62b07bf097baf26b416c815aea514e05c9ad6d63` | `C3CBC81D498175FD2AA69EE2DFE7DAFBF519052A96283C2568FE5B3B16618456` | `libavcodec.so`、`libavdevice.so`、`libavfilter.so`、`libavformat.so`、`libavutil.so`、`libswresample.so`、`libswscale.so`、`libffmpegkit.so`、`libffmpegkit_abidetect.so`、`libc++_shared.so` |
| libmpv JNI | `Riteshp2001/mpvlibAndroid` release/tag `2026-06-25`，commit `168e0a5e43b37c85509050cddcb5eaddc2e313c0` | `CA0D1C60DDFE5BAD46C369D0D0BF6C2D1A8BB5EC5A3AB916C33F892D0263A75E` | 薄 AAR 只保留 `libmpv.so`、`libplayer.so`、Java API 与必要 assets |

- 两个上游制品均使用 FFmpeg `n8.1.2`；`libmpv.so`/`libplayer.so` 所需的 FFmpeg 动态符号必须由同一
  FFmpegKit AAR 全部提供。
...
- 薄 AAR 是对固定输入的可复现结构变换，不是 Gradle packaging 选取。严禁 `pickFirst`、native
  `exclude` 或同名库覆盖。
...
- 初始化前固定 `vo=gpu`、`gpu-context=android`、所选 `hwdec`、AudioTrack 输出、缓存、字幕字体目录与
  默认倍速；不写隐式播放器替代路径。
- 网络 request 将真实 `User-Agent`、`Referer`、`Origin`、`Cookie`、`Range` 与 `Accept` 交给 mpv；
  缺失字段保持缺失，不根据页面 URL 猜测。
