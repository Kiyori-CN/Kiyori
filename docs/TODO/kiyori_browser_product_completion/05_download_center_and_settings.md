...
- 文件下载器根标题当前为与设置首页入口一致的“文件下载器”；旧“下载器及自定义”和
  “文件下载器设置”均不再作为顶部标题
...
- 顶部严格保留“我的下载”、四竖线菜单、“新增”“清空”；标题使用与其他浏览器子抽屉一致的
  `WebSessionDrawerHeader` 和绿色下载语义徽标；四竖线使用标题后动作槽位，视觉紧邻“载”字，
  同时保留 `36dp` 完整点击热区，把按压反馈裁剪为圆形，并让四竖线在圆形反馈内水平和垂直
  严格居中，避免正方形阴影；菜单包含排序方式、批量删除、文件管理、显示时间、分类显示和更多设置
...
- 排序、新增、完整链接、删除确认、条目长按操作和重命名/修改后缀全部使用共享语义弹窗容器：
  绿色或危险红色标题徽标、统一 `20dp` 圆角与细描边、标题分隔线、阴影和全窗口遮罩；动作矩阵和
  manager consumer 保持不变
...
- “我的下载”标题、菜单和长按操作弹窗与其他浏览器抽屉/弹窗保持同一视觉层级，并在窄屏不遮挡动作
