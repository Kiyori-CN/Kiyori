...
> 2026-08-14 当前状态：网络日志与广告拦截、标记广告和网页元素长按已完成本地实现；真实网页、
> 设备触控和订阅兼容性保持 `verification_pending`。

## 原则

以旧版四行菜单顺序、图标、尺寸和行为为参考；hikerView 的广告拦截与网页元素工作台只作为
...
- “网络日志”继续由当前 WebSession 持有，并在 `其他` 右侧增加独立的 `拦截` 筛选；列表、详情
  和操作弹窗显示客户端命中的规则来源与规则文本
- “标记广告”从浏览器菜单进入固定 `320dp` 底部工作台；工作台是实时 WebView 下方的 sibling，
  不覆盖页面内容。上栏为“拦截规则 / 节点 / HTML / 父 / 兄 / 弟 / 子 / 关闭”，下栏为
  “保存规则 / 编辑规则 / 预览 / 重置 / 清除拦截 / 拦截网站跳转”
- 编辑规则使用独立编辑弹窗；清除拦截使用确认/取消弹窗；网站跳转策略使用底部抽屉，提供
  “默认 / 允许跳转 / 跳转前询问 / 拦截跳转 / 取消”。进入标记模式默认锁定网页点击，
  选择元素不会触发链接导航；用户显式选择允许或询问策略后才改变跳转行为
- 保存规则写入当前站点的元素规则并立即应用；清除只操作 `BrowserAdBlockStore` 中匹配当前
  站点与 selector 的自定义元素规则，不创建第二规则 owner
- 网页元素长按按真实能力显示“新窗口打开 / 后台打开 / 复制链接 / 复制文本 / 外部打开 /
  选择文本 / 拦截网页元素 / 拦截过滤网址”；没有链接、资源或文本时不显示对应动作
- 广告规则统一由 `BrowserAdBlockStore` 持有，包含总开关、网址规则、元素规则、站点白名单和
  显式订阅；订阅只在用户点击刷新时下载，解析失败与忽略行形成明确状态
