...
- [2026-08-03 倍速、长按分段与悬浮控制层对齐](#2026-08-03-倍速长按分段与悬浮控制层对齐)
- [本轮定位](#本轮定位)
- [2026-07-28 全屏播放器 1:1 界面复刻计划](#2026-07-28-全屏播放器-11-界面复刻计划)
- [本地结果](#本地结果-1)
- [固定参考与差异](#固定参考与差异)
- [运行时修复设计](#运行时修复设计)
- [唯一会话与文件职责](#唯一会话与文件职责)
...
## 2026-08-03 倍速、长按分段与悬浮控制层对齐

[LOCAL DONE / DEVICE PENDING]

- 全屏与悬浮播放器共用同一 `PlayerSpeedMenu`。固定项从上到下为
  `3.0x / 2.75x / 2.5x / 2.25x / 2.0x / 1.75x / 1.5x / 1.25x / 1.0x /
  0.75x / 0.5x / 0.25x`，单项最小高度压缩到 `36dp`。
- 自定义倍速接受 `0.00..3.00` 且最多两位小数。非零值以百分之一为唯一合法精度直接进入唯一
  `PlayerSession` / `:player` runtime；`0.00x` 调用暂停，保留恢复播放时使用的原非零倍速。
- 未持久化过的“双击手势”默认值从跳转改为暂停/播放；已有明确设置继续按持久化值执行。
- 长按加速不再沿固定列表逐档提升：当前速度 `<1.0x` 临时到 `1.0x`，`1.0x..<2.0x` 到
  `2.0x`，`2.0x..<3.0x` 到 `3.0x`。松手、取消、会话变化与原有冲突路径继续恢复精确原速且
  不写倍速记忆。
- 悬浮播放器继续保持内容区全宽 `16:9`，按比例投影横屏顶部/侧边/底部结构，并与全屏共用
  `#7792FF -> #9A7BFF` 进度渐变、白色 `0.8` 轨道、蓝色滑块、三段上下遮罩和倍速弹窗。
  网速显示直接读取唯一 `PlayerSessionState.networkSpeedBytesPerSecond`，删除小窗自己的全局流量轮询。
- 聚焦 `PlayerPolicyTest 24/24` 与 `PlayerControlsPolicyTest 3/3` 已通过；倍速弹窗滚动、数字输入法、
  双击/长按手势和悬浮/全屏像素仍需目标设备验收。
...
- 双击手势：未持久化设置默认暂停/播放，也可选择左右半屏快退/快进；左右跳转使用独立秒数
- 长按加速：开启后按当前倍速区间临时到 `1.0x / 2.0x / 3.0x`，松手或手势取消恢复原速，
  不写入倍速记忆
- 按钮快进/快退时长：只约束播放器按钮
