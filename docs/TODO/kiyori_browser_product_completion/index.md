...
4. [DONE] P1：已按旧版 `5/5/2/5/6` 分组重新复刻网页浏览器设置；自定义主页、网页外部应用、网页定位和自动悬浮播放接入唯一 owner，搜索栏视频资源球由候选抽屉中的独立开关持有，其他旧版未实现项为空占位；根页面已使用无描边分组卡和连续折叠吸顶标题，本地测试与门禁通过，最新 Debug APK 与真机视觉状态见第十阶段
...
- 顶部“搜索栏嗅探入口”和“自动悬浮播放”分别控制搜索框视频资源球与推荐候选自动悬浮
...
- 人工播放固定进入已有 `sensorLandscape` 全屏 Activity，自动推荐才进入悬浮；播放器无法播放的问题另行修复
