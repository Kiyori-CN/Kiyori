...
8. [DONE] P0：阶段 11 的运行时、设置、浏览器候选与 native 门禁已完成；全屏播放器按用户 `2800x1260` 横屏和 `1260x2800` 竖屏截图继续收口，竖屏顶部沿用 legacy 固定标题/权重单元，底部九键等宽排列。顶部四个功能入口统一为字幕、弹幕、音轨、画面模式描边图标，位置不变且内部图标为 `24dp`；网速/单位和电量/时间完整显示且上下行字号字重一致。浏览器悬浮播放器为内容区全宽、固定 `16:9`、零边距，并与全屏共用唯一 session 和快进快退设置。`2026-07-28 19:18:21 +08:00` 的必现闪退纠正了播放器启动契约；`2026-07-29 14:31 +08:00` 的用户报告进一步确认 HTTPS MP4 已能进入 `ACTIVE` 并持续播放。随后控制层修复了播放进度更新取消 pointer detector 的根因：单击恢复/隐藏、双击、seek、亮度、音量、弹窗、进度拖动、三秒自动隐藏与锁定解锁现在由稳定状态机协调。没有真实 owner 的弹幕显示为禁用，更多菜单只保留查看日志；播放器设置页改为 `5/2/2/5` 四组 14 项真实 `PlayerSettingsStore` 配置。播放器日志汇总主进程和独立 runtime 的 MPV verbose、命令与错误，并使用屏幕内固定分区、最新在前结构化列表和 8 个单行横滑分类，关闭固定在标题栏，清空/复制/导出固定在底部。自动检查和 Debug APK 证据以阶段 11 最新记录为准；本地/在线视频的控制层真机交互、HLS 与更多站点仍保持 `verification_pending`
