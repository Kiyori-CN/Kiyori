...
- 顺序：审查源码、测试与文档 → 定向下载策略测试 → 形成候选提交并串行构建 Debug APK → 核验签名、包信息、对齐与 SHA-256 → 推送 `main`，同步预发布标签、说明与附件。README 同步内测下载入口；保持 `0.1.0 / 45` 开发版本和原 Debug 签名。
- 回滚依据：替换前保留原标签提交、发布说明与已校验旧 APK。远端结果以实际 ref 和附件 digest 验证，记录在发布条目及任务日记；设备与真实下载验收仍为 `verification_pending`。

## 2026-09-14 新建下载弹窗

- 基线 `main / 8257fed183dd03f131393b3064b2694d6fab4f04`；保留本轮未提交的扩展标题及文档改动。
- 目标：重整链接、文件命名、下载方式与主操作层级；完善主动粘贴、长链接、键盘、错误反馈、提交与命名校验。复用 BrowserDownloadManager、系统 DownloadManager、当前设置和 WebSession 请求身份，不增加网络探测、第二下载器或自动降级。
- 方案：公共模态容器，固定标题和底部主按钮，中间独立滚动；链接主输入区、按需展开命名、引擎说明卡片。共享手动命名校验，失败保留草稿，提交接受后关闭；系统任务不切换到内置下载中列表。
- 阶段：① 调用链和设计；② 实现、清理旧输入与完整链接二级弹窗；③ URL/命名回归与差异审查；④ 串行 Debug APK 构建。设备视觉、键盘与真实下载保持 `verification_pending`；不提交、不推送、不操作设备。
- 风险与回滚：HTTP(S) 签名参数必须原样交给下载层，长链接不得静默拼接或改写；文件名按 UTF-8 长度检查；更改扩展名不等于格式转换。无持久化格式迁移，回滚限本轮表单、手动输入策略和接线。
- 状态：本地实现与构建完成，设备 `verification_pending`。2 个定向 JVM 套件共 41 项通过（3m12s），失败/错误/跳过均为 0；覆盖无路径域名、编码文件名、非法地址、复合扩展名、路径字符及 UTF-8 文件名上限。完整架构检查 `m03` 通过，无快照调整；文档检查 521 文件、0 问题，差异审查通过。
- 最终串行 Debug 构建通过（2m24s，238 tasks），唯一 launcher、脚本代理与播放器打包检查通过。APK 生成于 `2026-09-14 06:05:02 +08:00`，488291558 bytes，`com.kiyori / 45 / 0.1.0`，V2 单 signer 与 16 KiB ZIP 对齐通过；SHA-256 `897250D95726978D0D25765D99E44577F51D68125237A2CB1B8F2AB7440BA52E`。
- 最终产物包含本轮扩展标题补充；12 个源码、测试和文档文件保留在本地，未提交推送，terminal 未改动。未运行 Lint、Release 或设备操作；待验收常规/窄屏/大字体、键盘避让、主动粘贴、旋转保留输入、失败修正、内置及系统下载和 M3U8 实际行为。

## 2026-09-14 扩展抽屉标题补充

- 基线：`main / 8257fed183dd03f131393b3064b2694d6fab4f04`，开始时工作区干净。
- 目标与方案：移除“扩展”标题区域脚本总数，加号菜单复用公共同行标题布局并靠右；仅调整展示，不改变新增、导入、搜索、分类或脚本运行逻辑。
- 阶段与验收：核对标题及加号菜单接线 → 最小修改与差异审查 → 串行 Debug APK 构建核验；本轮不提交、不推送。风险为窄屏和大字体排版，设备视觉与触控保持 `verification_pending`；回滚仅需撤回本节对应展示改动，无数据迁移。
- 本地验证：`git diff --check` 与文档检查通过（521 文件、0 问题）；串行 Debug 构建通过（1m44s，238 tasks），唯一 launcher、脚本代理和播放器打包检查通过。APK 生成于 `2026-09-14 05:48:35 +08:00`，488291558 bytes，`com.kiyori / 45 / 0.1.0`，SHA-256 `5499778104409E5485710C415D3C7BDB673709F4B8B9C586A2E69DA1A423207B`。改动保留在本地，未提交推送。
...
4. [LOCAL DONE / VERIFICATION PENDING] P1：网页浏览器设置当前为 `2/4/3/3/2/1/5/3` 八组 23 行：“内容过滤 / 网页扩展与脚本 / 主页与导航 / 启动与窗口 / 网页显示 / 网页交互 / 网站权限与数据 / 音视频嗅探”。页面顶部明确全局能力上限；内容过滤复用唯一 `BrowserAdBlockStore` 的总开关与管理入口，网站密码总开关和管理入口职责分离。浏览器菜单第三行第五个“网站配置”已成为冻结当前 HTTP(S) 完整 host 的真实可拖动抽屉，以 `globalEnabled && !siteDisabled` 为唯一优先级，提供广告拦截、用户脚本、返回缓存、左右滑动前进后退、缩放、长按、外部应用、定位、密码、嗅探入口和自动悬浮十一项负向开关并持久化；广告继续使用原白名单 owner，其余十项由 `WebSessionBrowserSettingsStore` 精确 host 规则持有。主源码编译和定向 JVM 已通过，最终门禁与本轮 Debug APK 证据以 [历史证据索引](../history/README.md) 为准。目标设备上的布局、抽屉拖动、系统 Back、真实站点行为、重启持久化、普通/无痕和后台导航现场矩阵保持 `verification_pending`
