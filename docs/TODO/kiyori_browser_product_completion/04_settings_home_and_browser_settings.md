...
- 当前八组为 `2/4/3/3/2/1/5/3`：内容过滤、网页扩展与脚本、主页与导航、启动与窗口、网页显示、
...
- 浏览器设置当前七组为 `4/3/3/2/1/4/3`：网页扩展与脚本、主页与导航、启动与窗口、网页显示、
...
- 打开扩展中心、脚本诊断或脚本详情时，设置会话以 `BrowserWorkspaceReturnToken` 暂停；关闭工作台
...
- 当前浏览器设置从 `4/4/3` 三组 11 行调整为 `4/2/2/4/3` 五组 15 行：
  “网页扩展与脚本 / 主页与导航 / 网页显示 / 网站权限与数据 / 音视频嗅探”。
...
- 当前浏览器设置只保留 `4/4/3` 共 11 个真实选项：“网页扩展与脚本 / 主页与网站数据 /
  音视频嗅探”；历史标签手势、翻译、密码、X5、代理等无 owner 占位不再显示。
- “网页扩展与脚本”只保留允许用户脚本、扩展中心、扩展权限与网站范围、脚本诊断与日志四项；
  已安装、当前页、更新和日志继续是同一 userscript 工作台中的标签。
- 权限页只显示运行状态说明、总授权和逐脚本声明范围；空态与运行环境不使用不可点击的伪导航行。
- 从 Browser Home 打开设置后，扩展中心、脚本诊断和权限页脚本详情先关闭 Browser Settings child，
...
- 当前设置结构已由同日插件封板收敛为 `4/4/3`；“自动悬浮最小时长”仍是“音视频嗅探”第三项。
...
本里程碑记录 2026-07-25 曾完成的浏览器设置实现与构建证据。2026-07-26 的[专业主题第七阶段](../kiyori_professional_browser_theme/07_ai_theme_settings_and_browser_settings_removal.md)已删除浏览器设置页、搜索记录管理子页、Shell 路由、外部 Intent 和页面专用逻辑。浏览器菜单第四行“浏览器设置”按钮、原图标和三按钮位置继续保留，点击为空；下文只作为历史记录，不再描述当前源码。
