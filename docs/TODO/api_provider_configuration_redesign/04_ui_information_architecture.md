...
供应商选择器使用共享 `KiyoriModalBottomDrawer`，保留搜索、可访问性语义、圆角 Surface 和
当前选中态，并使用以下顺序：
...
多协议选择器使用共享 `KiyoriModalBottomDrawer`；有限高度内只有协议列表可滚动。列表只包含
当前供应商 catalog 明确声明的协议，用户点击后立即更新当前 Compose 草稿并关闭抽屉。不存在
“自动识别”、检测结果或检测失败状态，也不在连接测试或正式聊天请求中尝试其他协议。
