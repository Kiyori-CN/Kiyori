...
- [8.1 当前进度与证据等级](#81-当前进度与证据等级)
- [8.2 用户现场测试基线](#82-用户现场测试基线)
- [8.3 已确认缺陷一：超时会被误分类为 `REQUEST_CANCELLED`](#83-已确认缺陷一超时会被误分类为-request_cancelled)
- [8.4 并发、排队与生命周期的当前结论](#84-并发排队与生命周期的当前结论)
- [8.5 已确认缺陷二：`MODEL_CONFIG` 的产品语义与 UI 不一致](#85-已确认缺陷二model_config-的产品语义与-ui-不一致)
- [8.6 已确认缺陷三：搜索结果卡片缺少统一的分层折叠状态](#86-已确认缺陷三搜索结果卡片缺少统一的分层折叠状态)
- [8.7 已确认缺陷四：专用 evidence UI 解析失败被静默降为普通结果](#87-已确认缺陷四专用-evidence-ui-解析失败被静默降为普通结果)
- [8.8 流式支持的当前判断](#88-流式支持的当前判断)
- [8.9 `User-Agent` 的当前判断](#89-user-agent-的当前判断)
- [8.10 其他已发现优化点](#810-其他已发现优化点)
- [8.11 检查点后的调查清单与转交状态](#811-检查点后的调查清单与转交状态)
- [8.12 本检查点创建时未完成的内容](#812-本检查点创建时未完成的内容)
- [8.13 后续调查收敛结果](#813-后续调查收敛结果)
...
生产诊断字段才能证明的项目已经转入
[优化实施计划的测试矩阵](09_20260813_optimization_implementation_plan.md#913-测试矩阵)，本轮没有
修改或运行这些实现级测试。
...
[配置架构：只保留 PACKAGE_ENV](09_20260813_optimization_implementation_plan.md#96-配置架构只保留-package_env)。
...
[流式与 background 后续里程碑](09_20260813_optimization_implementation_plan.md#911-流式与-background-后续里程碑)。
...
[`User-Agent` 与 headers](09_20260813_optimization_implementation_plan.md#910-user-agent-与-headers)。
...
实施顺序、测试矩阵与验收门禁已转入
[优化实施计划](09_20260813_optimization_implementation_plan.md)。
