...
- [1. 文档定位与任务契约](#1-文档定位与任务契约)
- [2. 当前基线与证据等级](#2-当前基线与证据等级)
- [3. 修复前与当前正常时序](#3-修复前与当前正常时序)
- [4. 修复前失效时序](#4-修复前失效时序)
- [5. 单一回合与消息状态 owner](#5-单一回合与消息状态-owner)
- [6. 完整回合状态机](#6-完整回合状态机)
- [7. 五类投影和数据边界](#7-五类投影和数据边界)
- [8. 终态和内容矩阵](#8-终态和内容矩阵)
- [9. 取消来源、竞态和停止协议](#9-取消来源竞态和停止协议)
- [10. MessageEntity 与 provider-private state 取舍](#10-messageentity-与-provider-private-state-取舍)
- [11. Provider-neutral replay history validator](#11-provider-neutral-replay-history-validator)
- [12. Provider adapter 协议矩阵](#12-provider-adapter-协议矩阵)
- [13. 已污染聊天的正式、可审计修复](#13-已污染聊天的正式可审计修复)
- [14. 编辑、删除、variant 和回滚后的失效规则](#14-编辑删除variant-和回滚后的失效规则)
- [15. 进程重启规则](#15-进程重启规则)
- [16. UI、错误反馈和无障碍](#16-ui错误反馈和无障碍)
- [17. 审计事件和脱敏](#17-审计事件和脱敏)
- [18. 精确影响文件清单](#18-精确影响文件清单)
- [19. 分阶段实施顺序、回滚点和门槛](#19-分阶段实施顺序回滚点和门槛)
- [20. 验证矩阵](#20-验证矩阵)
- [21. 不变量和禁止方案](#21-不变量和禁止方案)
- [22. 当前结论、下一步和状态标记](#22-当前结论下一步和状态标记)
- [2026-09-05 DeepSeek Chat HTTP/2 CANCEL 诊断与连接隔离](#2026-09-05-deepseek-chat-http2-cancel-诊断与连接隔离)
...
- docs/doc-src/documentation_guide.md、项目 AGENTS.md 和正式开发准备清单仍高于本文。
