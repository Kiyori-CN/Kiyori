...
默认主题应像成熟的 AI 浏览器，而不是营销页面或彩色工具箱。背景、长文本和大面积 surface 保持中性，蓝色只承担动作、选择、用户消息和设置控件的语义。浏览器 chrome 继续以黑白灰承载网页，不读取应用强调色。
...
Software Home 的五色搜索框和固定蓝色分段强调属于品牌装饰。负一屏、文件管理首页和设置首页的灰底、白卡、分类渐变及多彩图标属于信息层级。两类固定页面色板都不写入应用 ColorScheme，也不随用户 primary/secondary 改色。
...
影响 AI、AI 与系统设置、通用应用控件和软件壳选中态；不影响浏览器主题和固定页面装饰色。
