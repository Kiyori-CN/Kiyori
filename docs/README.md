...
> 本页是文档导航，不是新的事实来源。具体事实以表格中标注的权威文档为准。
...
新增、移动、归档或重写文档前，请阅读 [文档维护规范](doc-src/documentation_guide.md)。结构、命名、README 修改说明和验证入口均在该页维护。

目录采用两层导航：本页按读者目标分流，[完整目录](CATALOG.md)列出父仓全部文档；专项的
`index.md` 持有进度与验收。长文通过页内目录定位，历史证据从 [历史索引](TODO/history/README.md) 查询。
文件夹按实际职责建立，不预建空目录；脚本与 ToolPkg 的公开指南路径、协议夹具和上游许可证保持稳定。

根入口有明确分工：`README.md` 解决“用户如何开始”，`CONTEXT.md` 解决“项目使用哪些术语和不变量”，`AGENTS.md` 解决“维护者如何工作”。当一段文字同时适合多个入口时，保留一份权威正文，其余入口只提供短说明和链接。
