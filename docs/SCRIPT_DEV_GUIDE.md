...
不要把“开发插件”和“修改 Kiyori 主仓”混为一件事。两者使用不同的目录、资料和调试路径。
...
只有需要修改 Kiyori 应用本身、内置示例或 Android 宿主能力时，才进入主仓开发流程。这要求克隆主仓、安装依赖并使用 ADB 调试工具，不能把它当作普通插件作者的默认起点。
...
本节只面向需要改动 Kiyori 主仓的贡献者。普通插件作者请使用前述应用内 Skill 工作流或自己的独立插件仓库。
