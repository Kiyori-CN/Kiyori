...
class OperitDataDocumentsProvider : DocumentsProvider() {
    ...
    override fun onCreate(): Boolean {
        return try {
            ...
            if (!dataRootCanonical.isDirectory) {
                throw FileNotFoundException("Kiyori data directory not found: $dataRootCanonicalPath")
            }
    ...
    override fun queryRoots(projection: Array<out String>?): Cursor {
        ...
        row.add(DocumentsContract.Root.COLUMN_TITLE, "Kiyori Data")
    ...
    override fun deleteDocument(documentId: String) {
        if (documentId == DOC_ID_ROOT) {
            throw IllegalArgumentException("Cannot delete Kiyori data root")
        }
    ...
    override fun renameDocument(documentId: String, displayName: String): String {
        if (documentId == DOC_ID_ROOT) {
            throw IllegalArgumentException("Cannot rename Kiyori data root")
        }
    ...
    override fun moveDocument(
        ...
        if (sourceDocumentId == DOC_ID_ROOT) {
            throw IllegalArgumentException("Cannot move Kiyori data root")
        }
    ...
    private fun ensureInsideDataRoot(file: File): File {
        ...
        if (!isSameOrChild(dataRootCanonical, canonical)) {
            throw SecurityException("Path escapes Kiyori data root: ${canonical.path}")
        }
        return canonical
    }
