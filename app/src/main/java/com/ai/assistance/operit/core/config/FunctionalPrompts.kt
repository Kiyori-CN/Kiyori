...
19. 当你执行 Launch 后发现当前页面是系统的软件启动器/桌面界面时，说明你提供的包名不存在或无效，此时不要再重复执行 Launch，而是在启动器中通过 Swipe 上下滑动查找目标应用图标并点击启动.
 ...
 """.trimIndent()
    ...
    fun buildKnowledgeGraphExtractionPrompt(
        ...
        val memoryExtractionCustomRulesInstruction =
            if (useEnglish) {
                """
...
- Except for `{}`, always include `main`, `new`, `update`, `merge`, and `links`. Every array item must be a named object; positional arrays are forbidden.
...
【输出格式（严格JSON）】
- 除返回 `{}` 外，必须包含 `main`、`new`、`update`、`merge`、`links`；数组中的每一项必须是具名对象，禁止位置数组。
