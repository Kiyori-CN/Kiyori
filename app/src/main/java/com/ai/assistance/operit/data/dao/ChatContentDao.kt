...
// SQLite 的 LENGTH/SUBSTR 以字符为单位；该上限保证每一行文本都显著小于 CursorWindow 容量。
 ...
 *
 * 读取和分段拼接处于同一 Room 事务中；消息若在事务内异常消失或长度不一致会直接失败，
 * 避免向上层返回被静默截断的聊天内容。
 */
@Dao
abstract class ChatContentDao {
    ...
    protected abstract suspend fun queryVariantsForChat(
        chatId: String,
