...
const BraveSearch = (function () {
    ...
    function configuredKeys(category, required) {
        const envName = ENV_BY_CATEGORY[category];
        if (!envName)
            throw new Error("Unknown Brave key category: " + category);
    ...
    function nextStartIndex(category, keyCount) {
        return Number(Java.callStatic("com.ai.assistance.operit.core.tools.javascript.ApiKeyRoundRobin", "nextIndex", "brave_search:" + ENV_BY_CATEGORY[category], keyCount));
    }
    ...
    function cleanParams(params, reserved) {
        const result = {};
    ...
    async function execute(spec, key) {
        ...
        if (spec.method === "POST")
            builder = builder.jsonBody(params);
        const request = builder.build();
        if (spec.stream)
            return client.streamExecute(request, () => undefined);
        return client.execute(request);
    }
    function isKeyError(statusCode) {
        return [401, 402, 403, 429].indexOf(statusCode) >= 0;
    }
    async function requestWithRotation(category, spec, preferredKeyIndex) {
    ...
    function querySpec(path, q, params, requestedMethod, headers) {
        return {
    ...
    async function llmContext(params) {
    ...
    async function localPois(params) {
    ...
    async function poiDescriptions(params) {
    ...
    async function richSearch(params) {
    ...
    async function summarizer(params) {
        ...
        return runTool("summarizer", () => {
            if (allowed.indexOf(resource) < 0)
                throw new Error("Unsupported Summarizer resource: " + resource);
    ...
    function testSpec(category) {
        ...
        if (category === "suggest")
            return querySpec("/res/v1/suggest/search", "Kiyori", { count: 1 }, "GET");
        return querySpec("/res/v1/spellcheck/search", "Kiyori", {}, "GET");
    }
    async function testOneKey(category, key, keyIndex, allKeys) {
