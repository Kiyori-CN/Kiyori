...
const ticket12306 = (function () {
    ...
    class RailwayError extends Error {
        constructor(code, message, stage = 'input') {
            super(message);
            this.code = code;
            this.stage = stage;
        }
    ...
    let catalogPromise;
    function record(value, stage) {
        if (value === null || typeof value !== 'object' || Array.isArray(value))
            throw new RailwayError('INVALID_RESPONSE', '12306 响应结构异常。', stage);
        return value;
    }
    function text(value) { return typeof value === 'string' ? value : ''; }
    function required(value, field) {
        if (typeof value !== 'string' || !value.trim())
            throw new RailwayError('INVALID_ARGUMENT', `${field} 必须为非空字符串。`);
        return value.trim();
    }
    function formatDate(value) { return value.toISOString().slice(0, 10); }
    function today() { return formatDate(new Date(Date.now() + 8 * 3600000)); }
    function dateValue(value, field, pastAllowed = false) {
        const input = required(value, field);
        if (!/^\d{4}-\d{2}-\d{2}$/.test(input))
            throw new RailwayError('INVALID_ARGUMENT', `${field} 必须为 yyyy-MM-dd。`);
    ...
    function code(value, field) {
        ...
        if (!/^[A-Z]{3}$/.test(result))
            throw new RailwayError('INVALID_ARGUMENT', `${field} 必须为查询得到的三位车站编码。`);
        return result;
    }
    function names(value, field) {
        const parts = required(value, field).split('|').map(s => s.trim());
        if (parts.some(s => !s) || parts.length > 50)
            throw new RailwayError('INVALID_ARGUMENT', `${field} 需为 1–50 个非空名称，以 | 分隔。`);
        return [...new Set(parts)];
    }
    function options(params, transfer = false) {
        ...
        if (from === to)
            throw new RailwayError('INVALID_ARGUMENT', '出发站与到达站不能相同。');
        if (params.train_filter_flags !== undefined && typeof params.train_filter_flags !== 'string')
            throw new RailwayError('INVALID_ARGUMENT', 'train_filter_flags 必须为字符串。');
        const flags = (params.train_filter_flags ?? '').trim().toUpperCase();
        if (/[^GCDZTKOFS]/.test(flags))
            throw new RailwayError('INVALID_ARGUMENT', 'train_filter_flags 仅支持 G/C/D/Z/T/K/O/F/S。');
        const sort = params.sort_flag ?? '';
        if (!['', 'startTime', 'arriveTime', 'duration'].includes(sort))
            throw new RailwayError('INVALID_ARGUMENT', 'sort_flag 仅支持 startTime、arriveTime、duration。');
        if (params.sort_reverse !== undefined && typeof params.sort_reverse !== 'boolean')
            throw new RailwayError('INVALID_ARGUMENT', 'sort_reverse 必须为布尔值。');
        const limit = params.limited_num ?? (transfer ? 10 : 0);
        if (!Number.isSafeInteger(limit) || limit < (transfer ? 1 : 0) || limit > 1000 || (transfer && limit > 100))
            throw new RailwayError('INVALID_ARGUMENT', transfer ? 'limited_num 必须为 1–100 的整数。' : 'limited_num 必须为 0–1000 的整数，0 表示不限制。');
    ...
    async function request(url, stage, cookies = '', deadline = Date.now() + QUERY_DEADLINE_MS) {
        const remaining = deadline - Date.now();
        if (remaining <= 0)
            throw new RailwayError('QUERY_TIMEOUT', '查询已达到等待上限，请稍后重新查询。', stage);
        const headers = { 'User-Agent': 'Mozilla/5.0', Referer: stage.startsWith('transfer') ? TRANSFER_INIT : DIRECT_INIT };
        if (cookies)
            headers.Cookie = cookies;
        let response;
        try {
            response = await client(Math.min(20000, remaining)).newRequest().url(url).method('GET').headers(headers).build().execute();
        }
        catch (_error) {
            throw new RailwayError('NETWORK_ERROR', '12306 连接失败或超时，请检查网络后重试。', stage);
        }
        if (response.statusCode >= 300 && response.statusCode < 400) {
            const location = Object.entries(response.headers ?? {}).find(([name]) => name.toLowerCase() === 'location')?.[1] ?? '';
            if (/(?:\/passport\/|\/login[/.?]|\/login$)/i.test(location))
                throw new RailwayError('AUTH_REQUIRED', '12306 要求登录，请使用官方页面；本工具不会绕过登录。', stage);
            throw new RailwayError('REDIRECT_REJECTED', '12306 返回了未受支持的跳转，请在官网确认访问状态。', stage);
        }
        if (!response.isSuccessful())
            throw new RailwayError('HTTP_ERROR', `12306 返回 HTTP ${response.statusCode}。`, stage);
        return response;
    }
        ...
        for (const [key, value] of Object.entries(response.headers ?? {})) {
            ...
            for (const item of (Array.isArray(value) ? value : [value])) {
                for (const cookie of item.split(/,(?=\s*[^;,=\s]+=)/)) {
                    ...
                    if (split > 0)
                        result.set(pair.slice(0, split), pair.slice(split + 1));
                }
            }
        }
    ...
    function jsonResponse(response, stage) {
        let parsed;
        try {
            parsed = JSON.parse(response.content.replace(/^\uFEFF/, ''));
        }
        catch (_error) {
            throw new RailwayError('INVALID_RESPONSE', '12306 未返回 JSON，可能是访问校验页或服务异常；不能据此判断无票。', stage);
        }
        const data = record(parsed, stage);
        if (data.status === false || (typeof data.httpstatus === 'number' && data.httpstatus !== 200))
            throw new RailwayError('SERVICE_REJECTED', '12306 拒绝查询，请核对日期、预售期及官方服务状态。', stage);
        return data;
    }
    function initPath(content, transfer) {
        const match = content.match(transfer ? /\blc_search_url\s*=\s*['"]([^'"]+)['"]/ : /\bCLeftTicketUrl\s*=\s*['"]([^'"]+)['"]/);
        if (!match) {
            if (/passport|login|登录/i.test(content) && transfer)
                throw new RailwayError('AUTH_REQUIRED', '12306 中转查询当前要求登录，请使用官网；车站、直达和经停查询仍可独立使用。', 'transfer_init');
        ...
        if (!(transfer ? /^lcQuery\/query[A-Za-z0-9]*$/ : /^leftTicket\/query[A-Za-z0-9]*$/).test(path))
            throw new RailwayError('INVALID_RESPONSE', '官网返回了无法识别的查询路径。', 'init');
        return `/otn/${path}`;
    }
    async function catalog() {
        ...
        catalogPromise = (async () => {
            ...
            if (!match)
                throw new RailwayError('INVALID_RESPONSE', '无法读取官网车站表。', 'stations');
            ...
            for (const row of match[2].split('@').filter(Boolean)) {
                ...
                if (fields.length < 8 || !fields[1] || !/^[A-Z]{3}$/.test(fields[2]))
                    throw new RailwayError('INVALID_RESPONSE', '官网车站记录格式异常。', 'stations');
                const station = { station_id: `@${fields[0]}`, station_name: fields[1], station_code: fields[2], station_pinyin: fields[3], station_short: fields[4], station_index: fields[5], code: fields[6], city: fields[7], r1: fields[8] ?? '', r2: fields[9] ?? '' };
                ...
                if (station.city)
                    cities.set(station.city, [...(cities.get(station.city) ?? []), ref]);
            }
            if (!stations.size)
                throw new RailwayError('INVALID_RESPONSE', '官网车站表为空。', 'stations');
    ...
    async function checkStations(...codes) {
        const data = await catalog();
        if (codes.some(value => !data.stations.has(value)))
            throw new RailwayError('STATION_NOT_FOUND', '未找到车站编码，请先查询车站名称及编码。');
        return data;
    }
    function minutes(value, stage, clock = false) {
        const match = value.match(clock ? /^(\d{2}):(\d{2})$/ : /^(\d{1,3}):(\d{2})$/);
        if (!match || Number(match[2]) > 59 || (clock && Number(match[1]) > 23))
            throw new RailwayError('INVALID_RESPONSE', '车次时间格式异常。', stage);
        return Number(match[1]) * 60 + Number(match[2]);
    }
    function durationText(total) { return `${Math.floor(total / 60).toString().padStart(2, '0')}:${(total % 60).toString().padStart(2, '0')}`; }
    function seatStatus(value) {
        if (/^\d+$/.test(value))
            return Number(value) === 0 ? '无票' : `剩余${Number(value)}张票`;
        if (['有', '充足'].includes(value))
            return '有票';
        if (value === '无')
            return '无票';
        if (value === '候补')
    ...
    function seats(raw) {
        ...
        for (let i = 0; i + 10 <= priceText.length; i += 10) {
            ...
            if (seat && !prices.has(seat.short)) {
                prices.set(seat.short, Number(part.slice(1, 6)) / 10);
                priceNames.set(seat.short, seat.name);
            }
        }
        const groups = new Map([['swz', '商务座'], ['tz', '特等座'], ['zy', '一等座'], ['ze', '二等座'], ['gr', '高级软卧'], ['rw', '软卧'], ['yw', '硬卧'], ['rz', '软座'], ['yz', '硬座'], ['wz', '无座'], ['srrb', '动卧'], ['qt', '其他']]);
        return [...groups].flatMap(([short, name]) => {
            const num = text(raw[`${short}_num`]);
            if (!num && !prices.has(short))
                return [];
            return [{ seat_name: priceNames.get(short) ?? name, num, status: seatStatus(num), price: prices.get(short) ?? null }];
        });
    }
    function train(raw, date, stationNames) {
        ...
        if (durationMinutes <= 0 || (startMinutes + durationMinutes) % 1440 !== arriveMinutes)
            throw new RailwayError('INVALID_RESPONSE', '车次起止时间与历时不一致。', 'tickets');
        const id = text(raw.train_no), display = text(raw.station_train_code);
        if (!id || !display || !text(raw.from_station_telecode) || !text(raw.to_station_telecode))
            throw new RailwayError('INVALID_RESPONSE', '车次缺少标识或站点。', 'tickets');
    ...
    function trainMatches(item, flags) {
        return !flags || [...flags].some(flag => {
            if (flag === 'G')
                return /^[GC]/.test(item.start_train_code);
            if (flag === 'O')
                return !/^[GCDZTK]/.test(item.start_train_code);
            if (flag === 'F')
                return item.dw_flag.includes('复兴号');
            if (flag === 'S')
                return item.dw_flag.includes('智能动车组');
            return item.start_train_code.startsWith(flag);
        });
    }
    function filtered(items, opts, applyLimit = true) {
        ...
        if (opts.sort)
            result.sort((a, b) => {
                const key = (item) => opts.sort === 'duration' ? item.duration_minutes : Date.parse(`${opts.sort === 'startTime' ? item.start_date : item.arrive_date}T${opts.sort === 'startTime' ? item.start_time : item.arrive_time}:00Z`);
    ...
    function trainText(item) {
        ...
        if (item.can_buy === false)
            lines.push('- 当前不可在线预订，请以官网销售状态为准。');
        if (!item.seats.length)
            lines.push('- 座席余票及票价未提供，不表示无票。');
        for (const seat of item.seats)
            lines.push(`- ${seat.seat_name}: ${seat.status} ${seat.price === null ? '票价未提供' : `${seat.price}元`}`);
        return lines.join('\n');
    }
    ...
    async function get_stations_code_in_city(params) {
        ...
        if (!found)
            throw new RailwayError('CITY_NOT_FOUND', '未找到城市，请使用官方城市名或查询具体车站。');
        return found;
    }
    ...
    async function get_tickets(params) {
        ...
        if (typeof response.c_url === 'string') {
            ...
            if (!/^leftTicket\/query[A-Za-z0-9]*$/.test(next) || `/otn/${next}` === path)
                throw new RailwayError('INVALID_RESPONSE', '官网查询路径循环或无效。', 'direct_query');
            path = `/otn/${next}`;
        ...
        if (!Array.isArray(body.result))
            throw new RailwayError('INVALID_RESPONSE', '余票响应缺少车次列表。', 'direct_query');
        ...
        if (body.map && typeof body.map === 'object' && !Array.isArray(body.map))
            for (const [key, value] of Object.entries(body.map))
                if (typeof value === 'string')
                    mapping[key] = value;
        const all = body.result.map(row => {
            if (typeof row !== 'string' || row.split('|').length < 34)
                throw new RailwayError('INVALID_RESPONSE', '余票车次字段不完整。', 'direct_query');
            const fields = row.split('|');
    ...
    function transferRow(value, date) {
        const row = record(value, 'transfer_query');
        if (!Array.isArray(row.fullList) || row.fullList.length !== 2)
            throw new RailwayError('INVALID_RESPONSE', '中转方案必须包含两个行程段。', 'transfer_query');
        const trains = [];
        for (const value of row.fullList) {
            ...
            if (/^\d{8}$/.test(departure))
                departure = `${departure.slice(0, 4)}-${departure.slice(4, 6)}-${departure.slice(6)}`;
            if (!departure)
                throw new RailwayError('INVALID_RESPONSE', '中转方案缺少第二程上车日期，不能用始发日期代替。', 'transfer_query');
            const parsed = train(leg, dateValue(departure, 'middle_date', true), {});
            const previous = trains[trains.length - 1];
            if (previous && Date.parse(`${parsed.start_date}T${parsed.start_time}:00Z`) < Date.parse(`${previous.arrive_date}T${previous.arrive_time}:00Z`))
                throw new RailwayError('INVALID_RESPONSE', '中转方案的第二程早于第一程到达时间。', 'transfer_query');
            trains.push(parsed);
        }
        ...
        if (typeof suppliedDuration === 'number' && Number.isSafeInteger(suppliedDuration) && suppliedDuration > 0) {
            ...
            if (!match || (!match[1] && !match[2]))
                throw new RailwayError('INVALID_RESPONSE', '中转总历时缺失或格式异常。', 'transfer_query');
            duration = Number(match[1] || 0) * 60 + Number(match[2] || 0);
            if (!Number.isSafeInteger(duration) || duration <= 0)
                throw new RailwayError('INVALID_RESPONSE', '中转总历时必须为正整数分钟。', 'transfer_query');
        }
        ...
        if (total !== elapsed)
            throw new RailwayError('INVALID_RESPONSE', '中转总历时与两程日期时间不一致。', 'transfer_query');
    ...
    async function get_interline_tickets(params) {
        ...
        if (params.show_wz !== undefined && typeof params.show_wz !== 'boolean')
            throw new RailwayError('INVALID_ARGUMENT', 'show_wz 必须为布尔值。');
        const deadline = Date.now() + QUERY_DEADLINE_MS;
        ...
        const cursors = new Set(), seen = new Set(), rows = [];
        let pages = 0, scanComplete = false;
        while (pages < MAX_TRANSFER_PAGES && rows.length < MAX_TRANSFER_ROWS) {
            if (cursors.has(args.result_index))
                throw new RailwayError('PAGINATION_STALLED', '12306 返回重复分页游标，已停止查询。', 'transfer_query');
            ...
            if (!Array.isArray(body.middleList))
                throw new RailwayError('INVALID_RESPONSE', '中转响应缺少方案列表，不能据此判断无票。', 'transfer_query');
            pages++;
            let added = 0;
            if (body.middleList.length > MAX_TRANSFER_ROWS - rows.length)
                throw new RailwayError('RESPONSE_TOO_LARGE', '中转响应超过本次方案数量上限，请缩小查询范围。', 'transfer_query');
            ...
            if (scanComplete)
                break;
            if (!added)
                throw new RailwayError('PAGINATION_STALLED', '分页未返回新方案，已停止查询。', 'transfer_query');
            if (!opts.sort && filtered(rows, opts, false).length >= opts.limit)
                break;
            const next = body.result_index;
            if ((typeof next !== 'string' && typeof next !== 'number') || !/^\d+$/.test(`${next}`))
                throw new RailwayError('INVALID_RESPONSE', '中转响应缺少有效分页游标。', 'transfer_query');
    ...
    async function get_train_route_stations(params) {
        ...
        if (!/^[A-Za-z0-9]{8,20}$/.test(id))
            throw new RailwayError('INVALID_ARGUMENT', 'train_no 必须为余票结果中的内部车次编号，例如 240000G53106，不能使用 G531。');
        ...
        if (!Array.isArray(data.data))
            throw new RailwayError('INVALID_RESPONSE', '经停响应缺少车站列表。', 'route_query');
        return data.data.map(value => {
            const stop = record(value, 'route_query');
            if (!text(stop.station_name) || !/^\d+$/.test(text(stop.station_no)))
                throw new RailwayError('INVALID_RESPONSE', '经停站信息不完整。', 'route_query');
    ...
    async function wrap(fn, params) {
        ...
        try {
            ...
            const query = value !== null && typeof value === 'object' && 'text' in value && 'details' in value ? value : undefined;
    ...
    return {
        ...
        get_stations_code_in_city: (p) => wrap(get_stations_code_in_city, p),
        get_station_code_of_citys: (p) => wrap(get_station_code_of_citys, p),
        get_station_code_by_names: (p) => wrap(get_station_code_by_names, p),
        get_station_by_telecode: (p) => wrap(get_station_by_telecode, p),
        get_tickets: (p) => wrap(get_tickets, p),
        get_interline_tickets: (p) => wrap(get_interline_tickets, p),
        get_train_route_stations: (p) => wrap(get_train_route_stations, p),
        main: (p) => wrap(main, p),
    };
})();
