...
class RenderBatchCoordinatorTest {
    @Test
    fun `request while batch is pending is included without another input`() = runTest {
        var flushCount = 0
        val coordinator =
    ...
    @Test
    fun `request during flush is drained before coordinator becomes idle`() = runTest {
        var flushCount = 0
    ...
    @Test
    fun `tool xml tail mutation is rendered without another input`() = runTest {
    ...
    @Test
    fun `equal length block node replacement is rendered`() = runTest {
    ...
    @Test
    fun `equal length child node replacement is rendered`() = runTest {
