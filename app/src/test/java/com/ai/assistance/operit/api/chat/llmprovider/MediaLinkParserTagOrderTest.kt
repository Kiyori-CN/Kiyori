...
class MediaLinkParserTagOrderTest {
    ...
    @Test fun fileLinks_decodeXmlFilenameAndCanBeRemoved() {
        val input = "Before <link type=\"file\" id=\"pdf-1\" filename=\"report&amp;one.pdf\">PDF</link> After"

        assertEquals(
            listOf(MediaLinkTag("file", "pdf-1", "report&one.pdf")),
            MediaLinkParser.extractMediaLinkTags(input),
        )
        assertEquals("Before  After", MediaLinkParser.removeMediaLinks(input))
    }
}
