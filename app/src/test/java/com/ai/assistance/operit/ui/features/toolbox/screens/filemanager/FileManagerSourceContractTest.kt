...
class FileManagerSourceContractTest {
    ...
    @Test fun `screen retains shell back and a single lifecycle owned view model`() {
        ...
        assertTrue(screen.contains("KiyoriSettingsTheme { FileManagerContent(onBack, onOpenSettings, modifier, sessionViewModel, onOpenAiDialogue, onOpenBrowser, onOpenBrowserUrl) }"))
        assertTrue(screen.contains("FileManagerContentHost(viewModel, onOpenBrowserUrl)"))
        val contentHost = source("FileManagerContentHost.kt")
        assertTrue(contentHost.contains("FileManagerOpenKind.BROWSER"))
        assertTrue(contentHost.contains("Uri.fromFile(file)"))
        assertTrue(contentHost.contains("openBrowser(uri.toString())"))
        assertFalse(contentHost.contains("WebView("))
