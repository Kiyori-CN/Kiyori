...
```bash
./tools/adb/execute_js.sh path/to/your/script.js functionName '{"param1":"value1"}'
```
...
```bash
./tools/adb/run_sandbox_script.sh temp/probe.js @params.json
```
...
```cmd
tools\adb\execute_js_dir.bat <根目录> <入口相对路径> <函数名> [JSON参数|@参数文件]
```
...
### 4. `tools/example_packages/sync_example_packages.py` 是干什么的
...
```cmd
tools\adb\execute_js_dir.bat app\src\androidTest\js <你的入口相对路径> run "{}"
```
...
- 用 `tools/example_packages/sync_example_packages.py` 同步
- 再用 `execute_js.bat` 去调用相关调试工具，或者直接在应用里验证加载结果

也就是说：

- 运行器测试的是“这段 JS 在安卓 runtime 里能不能执行”
- `tools/example_packages/sync_example_packages.py` 测的是“这个 examples 包能不能进入真正的包加载链路”

## 和 `tools/example_packages/sync_example_packages.py` 怎么配合
...
1. 改 `examples/...`
2. 运行 `tools/example_packages/sync_example_packages.py`
...
这是最需要和 `tools/example_packages/sync_example_packages.py` 配合的场景。
