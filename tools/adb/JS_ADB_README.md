...
- `tools/adb/execute_js.bat` / `tools/adb/execute_js.sh`
...
- `tools/adb/execute_js_dir.bat` / `tools/adb/execute_js_dir.sh`
  The directory-entry launcher.
  Bundles and runs a JS entry from a directory, so local `require(...)` chains and test suites work.
- `tools/adb/run_sandbox_script.bat` / `tools/adb/run_sandbox_script.sh`
...
Windows:
```cmd
tools\adb\execute_js_dir.bat app\src\androidTest\js com\ai\assistance\operit\util\ttscleaner\main.js run "{}"
```
...
```cmd
tools\adb\execute_js_dir.bat <root_dir> <entry_relative_path> <function_name> [JSON params|@params_file]
```
...
```cmd
tools\adb\run_sandbox_script.bat temp\probe.js @params.json
```

If your code is naturally top-level script logic, this is usually simpler than wrapping everything in `exports.main = ...`.

### 4. What `tools/example_packages/sync_example_packages.py` is for
...
```cmd
d:\Code\prog\assistance\.venv\Scripts\python.exe d:\Code\prog\assistance\tools\example_packages\sync_example_packages.py --include sidebar_bing_action
```

To sync all syncable example items for broad validation:

```cmd
d:\Code\prog\assistance\.venv\Scripts\python.exe d:\Code\prog\assistance\tools\example_packages\sync_example_packages.py --mode test
```

To update local assets without device hot reload:

```cmd
d:\Code\prog\assistance\.venv\Scripts\python.exe d:\Code\prog\assistance\tools\example_packages\sync_example_packages.py --include sidebar_bing_action --no-hot-reload
```
...
```cmd
tools\adb\execute_js_dir.bat app\src\androidTest\js <your_entry_relative_path> run "{}"
```
...
- run `tools/example_packages/sync_example_packages.py`
...
- `tools/example_packages/sync_example_packages.py` tests whether the `examples/` package enters the real package-loading path correctly

## How To Use It With `tools/example_packages/sync_example_packages.py`
...
```cmd
tools\adb\execute_js.bat path\to\script.js main @params.json
```
...
1. Edit `examples/...`
2. Run `tools/example_packages/sync_example_packages.py`
...
This is the case that most needs `tools/example_packages/sync_example_packages.py`.
...
```cmd
tools\adb\execute_js_dir.bat app\src\androidTest\js com\ai\assistance\operit\core\tools\javascript\script_mode_contract\return_object.js run "{}"
```
