...
static __thread int64_t report_last_time;
static __thread int report_first_report;
static __thread int64_t keyboard_last_time;

FFmpegExecutionSharedState *ffmpeg_execution_shared_state(void)
{
    const FFmpegKitExecutionContext *context = ffmpegkit_execution_context();

    av_assert0(context);
    av_assert0(context->tool_state);
    return context->tool_state;
}

int ffmpeg_execution_shared_state_init(FFmpegExecutionSharedState *state)
{
    int ret;

    memset(state, 0, sizeof(*state));
    ret = ff_mutex_init(&state->vstats_lock, NULL);
    if (ret)
        return AVERROR(ret);

    state->vstats_lock_initialized = 1;
    atomic_init(&state->nb_output_dumped, 0);
    return 0;
}

void ffmpeg_execution_shared_state_capture(FFmpegExecutionSharedState *state)
{
    state->dts_delta_threshold = dts_delta_threshold;
    state->dts_error_threshold = dts_error_threshold;
    state->frame_drop_threshold = frame_drop_threshold;
    state->max_error_rate = max_error_rate;
    state->do_benchmark = do_benchmark;
    state->do_benchmark_all = do_benchmark_all;
    state->do_hex_dump = do_hex_dump;
    state->do_pkt_dump = do_pkt_dump;
    state->copy_ts = copy_ts;
    state->start_at_zero = start_at_zero;
    state->copy_tb = copy_tb;
    state->debug_ts = debug_ts;
    state->exit_on_error = exit_on_error;
    state->abort_on_flags = abort_on_flags;
    state->print_stats = print_stats;
    state->stdin_interaction = stdin_interaction;
    state->filter_complex_nbthreads = filter_complex_nbthreads;
    state->filter_buffered_frames = filter_buffered_frames;
    state->vstats_version = vstats_version;
    state->auto_conversion_filters = auto_conversion_filters;
    state->ignore_unknown_streams = ignore_unknown_streams;
    state->copy_unknown_streams = copy_unknown_streams;
    state->recast_media = recast_media;
    state->stats_period = stats_period;
    state->filter_nbthreads = filter_nbthreads;
    state->vstats_filename = vstats_filename;
    state->filter_hw_device = filter_hw_device;
    state->int_cb = int_cb;
    hw_device_execution_state_capture(state);
}

void ffmpeg_execution_shared_state_bind(void *opaque)
{
    const FFmpegExecutionSharedState *state = opaque;

    av_assert0(state);
    dts_delta_threshold = state->dts_delta_threshold;
    dts_error_threshold = state->dts_error_threshold;
    frame_drop_threshold = state->frame_drop_threshold;
    max_error_rate = state->max_error_rate;
    do_benchmark = state->do_benchmark;
    do_benchmark_all = state->do_benchmark_all;
    do_hex_dump = state->do_hex_dump;
    do_pkt_dump = state->do_pkt_dump;
    copy_ts = state->copy_ts;
    start_at_zero = state->start_at_zero;
    copy_tb = state->copy_tb;
    debug_ts = state->debug_ts;
    exit_on_error = state->exit_on_error;
    abort_on_flags = state->abort_on_flags;
    print_stats = state->print_stats;
    stdin_interaction = state->stdin_interaction;
    filter_complex_nbthreads = state->filter_complex_nbthreads;
    filter_buffered_frames = state->filter_buffered_frames;
    vstats_version = state->vstats_version;
    auto_conversion_filters = state->auto_conversion_filters;
    ignore_unknown_streams = state->ignore_unknown_streams;
    copy_unknown_streams = state->copy_unknown_streams;
    recast_media = state->recast_media;
    stats_period = state->stats_period;
    filter_nbthreads = state->filter_nbthreads;
    vstats_filename = state->vstats_filename;
    filter_hw_device = state->filter_hw_device;
    int_cb = state->int_cb;
    hw_device_execution_state_bind(state);
}

void ffmpeg_execution_shared_state_uninit(FFmpegExecutionSharedState *state)
{
    av_assert0(!state->vstats_file);
    if (state->vstats_lock_initialized) {
        ff_mutex_destroy(&state->vstats_lock);
        state->vstats_lock_initialized = 0;
    }
}
...
static __thread volatile int received_sigterm;
static __thread volatile int received_nb_signals;
static __thread atomic_int transcode_init_done;
static __thread volatile int ffmpeg_exited;
static __thread int64_t copy_ts_first_pts;

static void ffmpeg_reset_state(void)
{
    FFmpegExecutionSharedState *shared_state =
        ffmpeg_execution_shared_state();

    av_assert0(!shared_state->vstats_file);
    av_assert0(!progress_avio);
    av_assert0(!input_files);
    av_assert0(!output_files);
    av_assert0(!filtergraphs);
    av_assert0(!decoders);
    ...
    atomic_store(&shared_state->nb_output_dumped, 0);
    graphprint_reset_state();
}
...
sigterm_handler(int sig)
{
    ...
    received_nb_signals++;
    term_exit_sigsafe();
    if(received_nb_signals == 4) {
        ret = write(2/*STDERR_FILENO*/,
                    "Received > 3 system signals; waiting for hosted cleanup\n",
                    strlen("Received > 3 system signals; waiting for hosted cleanup\n"));
        if (ret < 0) { /* Do nothing */ };
    }
}
...
static int decode_interrupt_cb(void *ctx)
{
    return received_nb_signals > atomic_load(&transcode_init_done) ||
           ffmpegkit_cancel_requested(ctx);
}

__thread AVIOInterruptCB int_cb;
...
static void ffmpeg_cleanup(int ret)
{
    ...
    ff_mutex_lock(&shared_state->vstats_lock);
    if (shared_state->vstats_file) {
        if (fclose(shared_state->vstats_file))
            av_log(NULL, AV_LOG_ERROR,
                   "Error closing vstats file, loss of information possible: %s\n",
                   av_err2str(AVERROR(errno)));
        shared_state->vstats_file = NULL;
    }
    ff_mutex_unlock(&shared_state->vstats_lock);
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    if (!is_last_report) {
        ...
        if (((cur_time - report_last_time) < stats_period && !report_first_report) ||
            (report_first_report &&
             atomic_load(&ffmpeg_execution_shared_state()->nb_output_dumped) <
                 nb_output_files))
            return;
        report_last_time = cur_time;
    }
    ...
    ffmpegkit_forward_report(report_frame_number, report_fps, report_quality,
                             total_size, pts, bitrate, speed);

    report_first_report = 0;
}
...
{
    const FFmpegKitExecutionContext *execution_context =
        ffmpegkit_execution_context();
    int ret = 0;
    int64_t timer_start, transcode_ts = 0;

    ffmpeg_execution_shared_state_capture(
        ffmpeg_execution_shared_state());
    print_stream_maps();
    ...
    while (!sch_wait(sch, stats_period, &transcode_ts)) {
        ...
        if (received_nb_signals ||
            ffmpegkit_cancel_requested(execution_context))
            break;
...
int ffmpeg_execute(int argc, char **argv)
{
    ...
    ffmpegkit_execution_begin(globalSessionId);
    ret = ffmpeg_execution_shared_state_init(&shared_state);
    if (ret < 0) {
        ffmpegkit_execution_end();
        return ret;
    }
    ffmpegkit_execution_set_tool_state(&shared_state,
                                       ffmpeg_execution_shared_state_bind);
    execution_context = ffmpegkit_execution_context();
    ffmpegkit_set_program("ffmpeg", 2000);
    ffmpegkit_restore_log_callback();
    int_cb.callback = decode_interrupt_cb;
    int_cb.opaque = (void *)execution_context;
    ffmpeg_reset_state();
    ffmpegkit_opt_common_reset();

    ret = ffmpeg_options_init();
    if (ret < 0)
        goto finish;
    ...
    ret = (received_nb_signals ||
           ffmpegkit_cancel_requested(execution_context)) ? 255 :
...
finish:
    ...
    ffmpegkit_opt_common_uninit();

    ffmpegkit_restore_log_callback();
    av_log(NULL, AV_LOG_VERBOSE, "\n");
    av_log(NULL, AV_LOG_VERBOSE, "Exiting with exit code %d\n", ret);

    ffmpeg_execution_shared_state_uninit(&shared_state);
    ffmpegkit_execution_end();
