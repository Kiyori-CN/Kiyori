...
/*
 * FFmpeg 9 runs demux, decode, filter, encode and mux work on scheduler
 * threads.  The command options below are immutable once sch_start() is
 * reached, so one execution snapshots them here and every worker binds the
 * same snapshot.  Mutable cross-thread resources stay in this object rather
 * than becoming independent TLS copies.
 */
typedef struct FFmpegExecutionSharedState {
    ...
    FILE *vstats_file;
    AVMutex vstats_lock;
    int vstats_lock_initialized;
    atomic_uint nb_output_dumped;
} FFmpegExecutionSharedState;
...
void hw_device_execution_state_capture(FFmpegExecutionSharedState *state);
void hw_device_execution_state_bind(const FFmpegExecutionSharedState *state);
...
extern __thread AVIOInterruptCB int_cb;
