...
static int update_video_stats(OutputStream *ost, const AVPacket *pkt, int write_vstats)
{
    ...
    shared_state = ffmpeg_execution_shared_state();
    ff_mutex_lock(&shared_state->vstats_lock);

    /* this is executed just the first time update_video_stats is called */
    if (!shared_state->vstats_file) {
        shared_state->vstats_file = fopen(vstats_filename, "w");
        if (!shared_state->vstats_file) {
            perror("fopen");
            ret = AVERROR(errno);
            goto finish;
    ...
    if (vstats_version <= 1) {
        fprintf(shared_state->vstats_file,
                "frame= %5"PRId64" q= %2.1f ", frame_number,
                quality / (float)FF_QP2LAMBDA);
    } else  {
        fprintf(shared_state->vstats_file,
                "out= %2d st= %2d frame= %5"PRId64" q= %2.1f ",
                ost->file->index, ost->index, frame_number,
                quality / (float)FF_QP2LAMBDA);
    }

    if (psnr_val >= 0)
        fprintf(shared_state->vstats_file, "PSNR= %6.2f ", psnr_val);

    fprintf(shared_state->vstats_file, "f_size= %6d ", pkt->size);
    /* compute pts value */
    ...
    fprintf(shared_state->vstats_file, "type= %c\n",
