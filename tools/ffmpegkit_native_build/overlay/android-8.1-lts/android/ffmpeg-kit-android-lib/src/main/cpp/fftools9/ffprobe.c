...
static __thread int do_analyze_frames;
static __thread int do_bitexact;
static __thread int do_count_frames;
static __thread int do_count_packets;
static __thread int do_read_frames;
static __thread int do_read_packets;
static __thread int do_show_chapters;
static __thread int do_show_error;
static __thread int do_show_format;
static __thread int do_show_frames;
static __thread int do_show_packets;
static __thread int do_show_programs;
static __thread int do_show_stream_groups;
static __thread int do_show_stream_group_components;
static __thread int do_show_streams;
static __thread int do_show_stream_disposition;
static __thread int do_show_stream_group_disposition;
static __thread int do_show_data;
static __thread int do_show_program_version;
static __thread int do_show_library_versions;
static __thread int do_show_pixel_formats;
static __thread int do_show_pixel_format_flags;
static __thread int do_show_pixel_format_components;
static __thread int do_show_log;

static __thread int do_show_chapter_tags;
static __thread int do_show_format_tags;
static __thread int do_show_frame_tags;
static __thread int do_show_program_tags;
static __thread int do_show_stream_group_tags;
static __thread int do_show_stream_tags;
static __thread int do_show_packet_tags;

static __thread int show_value_unit;
static __thread int use_value_prefix;
static __thread int use_byte_value_binary_prefix;
static __thread int use_value_sexagesimal_format;
static __thread int show_private_data;

static __thread const char *audio_codec_name;
static __thread const char *data_codec_name;
static __thread const char *subtitle_codec_name;
static __thread const char *video_codec_name;

#define SHOW_OPTIONAL_FIELDS_AUTO       -1
#define SHOW_OPTIONAL_FIELDS_NEVER       0
#define SHOW_OPTIONAL_FIELDS_ALWAYS      1
static __thread int show_optional_fields;
...
static __thread int read_intervals_nb;

static __thread int find_stream_info;
...
static __thread EntrySelection selected_entries[FF_ARRAY_ELEMS(sections)];
...
static __thread const AVInputFormat *iformat;
static __thread const char *output_filename;
...
static __thread int log_print_prefix;
...
{
    ffmpegkit_log_set_replacement(log_callback_help, NULL);
    show_usage();
...
static int ffprobe_options_init(void)
{
    ...
    av_assert0(!options);
    options = av_memdup(local_options, sizeof(local_options));
    return options ? 0 : AVERROR(ENOMEM);
}
...
static void ffprobe_reset_state(void)
{
    av_assert0(!options);
    av_assert0(!audio_codec_name);
    av_assert0(!data_codec_name);
    av_assert0(!subtitle_codec_name);
    av_assert0(!video_codec_name);
    av_assert0(!output_format);
    av_assert0(!stream_specifier);
    av_assert0(!show_data_hash);
    av_assert0(!data_dump_format);
    av_assert0(!read_intervals);
    av_assert0(!input_filename);
    av_assert0(!print_input_filename);
    av_assert0(!output_filename);
    av_assert0(!nb_streams_packets);
    av_assert0(!nb_streams_frames);
    av_assert0(!selected_streams);
    av_assert0(!streams_with_closed_captions);
    av_assert0(!streams_with_film_grain);
    av_assert0(!log_buffer);
    ...
    log_buffer_size = 0;
    log_print_prefix = 1;
    memset(selected_entries, 0, sizeof(selected_entries));
}
...
int ffprobe_execute(int argc, char **argv)
{
    ...
    ffmpegkit_set_program("ffprobe", 2007);
    ...
    if (do_show_log)
        ffmpegkit_log_add_sink(log_callback, NULL);
