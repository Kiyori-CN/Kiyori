...
void *callbackThreadFunction() {
    ...
    while(atomic_load_explicit(&redirectionEnabled, memory_order_acquire)) {
...
static void enableNativeRedirection() {
    ...
    if (atomic_load_explicit(&redirectionEnabled, memory_order_acquire) != 0) {
        mutexUnlock();
        return;
    }
    atomic_store_explicit(&redirectionEnabled, 1, memory_order_release);
    ...
    if (rc != 0) {
        ...
        mutexLock();
        atomic_store_explicit(&redirectionEnabled, 0, memory_order_release);
        mutexUnlock();
        return;
    }
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    atomic_init(&redirectionEnabled, 0);
...
JNIEXPORT void JNICALL Java_com_arthenica_ffmpegkit_FFmpegKitConfig_disableNativeRedirection(JNIEnv *env, jclass object) {
    ...
    if (atomic_load_explicit(&redirectionEnabled, memory_order_acquire) != 1) {
        mutexUnlock();
        return;
    }
    atomic_store_explicit(&redirectionEnabled, 0, memory_order_release);
