...
__thread char *print_graphs_file;
__thread char *print_graphs_format;
__thread int auto_conversion_filters = 1;
__thread int64_t stats_period = 500000;

static __thread int file_overwrite;
static __thread int no_file_overwrite;
...
{
    av_assert0(!options);
    av_assert0(!vstats_filename);
    av_assert0(!filter_nbthreads);
    av_assert0(!print_graphs_file);
    av_assert0(!print_graphs_format);
