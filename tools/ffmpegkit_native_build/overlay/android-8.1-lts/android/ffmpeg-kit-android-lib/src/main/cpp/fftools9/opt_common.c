...
typedef struct OptCommonReportState {
    FILE *file;
    int file_level;
    int print_prefix;
    AVMutex mutex;
    int mutex_initialized;
} OptCommonReportState;

static __thread OptCommonReportState report_state;

#define report_file         report_state.file
#define report_file_level   report_state.file_level
#define report_print_prefix report_state.print_prefix
...
static __thread int warned_cfg;

void ffmpegkit_opt_common_reset(void)
{
    av_assert0(!report_file);
    av_assert0(!report_state.mutex_initialized);
    av_assert0(!ff_mutex_init(&report_state.mutex, NULL));
    report_state.mutex_initialized = 1;
...
void ffmpegkit_opt_common_uninit(void)
{
    if (report_state.mutex_initialized)
        ff_mutex_lock(&report_state.mutex);
    if (report_file) {
        fclose(report_file);
        report_file = NULL;
    }
    if (report_state.mutex_initialized) {
        ff_mutex_unlock(&report_state.mutex);
        ff_mutex_destroy(&report_state.mutex);
        report_state.mutex_initialized = 0;
    }
    report_file_level = AV_LOG_DEBUG;
    report_print_prefix = 1;
    warned_cfg = 0;
...
{
    av_assert0(program_name);

    if (!strcmp(program_name, "ffmpeg")) {
        show_help_default_ffmpeg(opt, arg);
    } else if (!strcmp(program_name, "ffprobe")) {
        show_help_default_ffprobe(opt, arg);
    } else {
        av_assert0(!"unknown embedded FFmpeg tool");
    }
}
...
{
    OptCommonReportState *state = ffmpegkit_log_sink_opaque();
    char line[1024];

    av_assert0(state);
    ff_mutex_lock(&state->mutex);
    av_log_format_line(ptr, level, fmt, vl, line, sizeof(line),
                       &state->print_prefix);
    if (state->file && state->file_level >= level) {
        fputs(line, state->file);
        fflush(state->file);
    }
    ff_mutex_unlock(&state->mutex);
}
...
int init_report(const char *env, FILE **file)
{
    ...
    ffmpegkit_log_add_sink(log_callback_report, &report_state);
