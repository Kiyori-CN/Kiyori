...
// Kiyori: keep TCP transfers alive across recoverable Linux syscall interruptions.
package bufio
...
func splice(source syscall.RawConn, destination syscall.RawConn, readCounters []N.CountFunc, writeCounters []N.CountFunc) (handed bool, n int64, err error) {
	...
	var writeSize int
	var writeErr error
	var readInterrupted, writeInterrupted bool
	readFunc := func(fd uintptr) (done bool) {
		for {
			p0, p1 := unix.Splice(int(fd), nil, pipeFDs[1], nil, maxSpliceSize, unix.SPLICE_F_NONBLOCK)
			// EINTR transfers no bytes. Retry the same syscall, never the application request.
			// Returning false here would wait for another readiness edge and can stall the stream.
			if p1 == unix.EINTR {
				if !readInterrupted {
					log.Print("[Kiyori splice] recovered EINTR direction=read")
	...
	writeFunc := func(fd uintptr) (done bool) {
		for writeSize > 0 {
			...
			if p1 == unix.EINTR {
				if !writeInterrupted {
					log.Print("[Kiyori splice] recovered EINTR direction=write")
