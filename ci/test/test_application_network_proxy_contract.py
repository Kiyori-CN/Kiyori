...
class ApplicationNetworkProxyContractTest(unittest.TestCase):
    ...
    def test_settings_route_and_environment_drawer_are_the_public_entry(self) -> None:
        ...
        self.assertIn("stringResource(R.string.pkg_env_network_proxy)", drawer)
        resources = (REPO_ROOT / "app/src/main/res/values/strings.xml").read_text(encoding="utf-8")
        self.assertIn('<string name="pkg_env_network_proxy">网络代理</string>', resources)
    ...
    def test_runtime_is_loopback_only_and_does_not_take_over_android_vpn(self) -> None:
        ...
        self.assertIn("EXTERNAL_DATA_RULE_TYPES", sanitizer)
        self.assertIn("isMihomoRuleExternalDataDependent", sanitizer)
        self.assertIn("return emptyMap()", sanitizer)
    ...
    def test_subscription_rules_and_site_proxy_disable_share_existing_owners(self) -> None:
        ...
        self.assertIn("NESTED_EXTERNAL_DATA_RULE_PATTERN", sanitizer)
