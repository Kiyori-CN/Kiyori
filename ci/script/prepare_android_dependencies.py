...
from prepare_mpv_player_dependency import (
    materialize_ffmpeg_player_dependency,
    materialize_mpv_player_dependency,
...
def synchronize_native_runtime(
    ...
    """Remove manual native copies now owned by resolved Maven dependencies."""

    if not android_ndk.is_dir():
        raise FileNotFoundError(f"Android NDK is missing: {android_ndk}")

    jni_root = repository / "app" / "src" / "main" / "jniLibs"
    for path in tuple(extracted_files):
        if path.is_relative_to(jni_root) and (
            path.name in STALE_JNILIB_NAMES or path.name == "libc++_shared.so"
        ):
            path.unlink()
...
def main() -> int:
    ...
    for removed in remove_retired_player_native_owners(repository):
        extracted_files.discard(removed)
    ...
    extracted_files.add(materialize_mpv_player_dependency(repository))
    extracted_files.add(materialize_ffmpeg_player_dependency(repository))
