...
| **Browser Home** | The immersive Kiyori browser root. It hides the five-item Kiyori bottom navigation and presents the shared Browser Runtime through browser-owned top-bar search, full-screen search, normal-window overview, tool-drawer, and bottom-bar chrome. |
...
- Browser Home's top bar is ordered Back, Search, and Refresh/Stop. Search opens a full-screen web search overlay without recreating the active WebView; its engine selection and recent records are browser-owned state. The bottom bar is ordered Back, Forward, Home, Tabs, and Toolbox. Tabs open a normal-window full-screen overview; history, bookmarks, downloads, userscripts, user-agent mode, network logs, page source, and tab-close actions remain browser-owned overlays rather than Kiyori App Shell destinations. Incognito, reader mode, ad marking, floating sniffing, and per-site configuration are not exposed without their corresponding runtime contracts.
