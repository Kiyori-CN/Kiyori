...
12. **文件管理显示偏好只有一份。** 设置与双栏的系统/手动隐藏开关、按环境与路径隔离的手动隐藏清单、列表大小共用 `FileManagerPreferences`；隐藏不修改文件本体。排序偏好只定义新会话默认值，当前排序/过滤归各窗格。搜索与任务历史由私有 `FileManagerHistoryStore` 保存，不保存权限或执行第二套任务。
