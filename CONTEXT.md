...
| **Full-Screen Web Search** | The shared web-only search surface opened from the Software Home Search Frame or Browser Home top bar. It shares the browser search-engine owner and normal-mode recent records, requires a real normal/incognito Profile control, and represents normal with an open eye and incognito with a closed eye. The control is a borderless standard icon action; mode changes show only a short white bottom message. Software Home always creates a new shared WebSession; Browser Home reuses the active tab only when its immutable Profile matches the selected Profile and otherwise creates a matching tab. It does not search bookmarks, browser history, files, mini apps, or AI conversations; those domains own their search pages. |
| **Settings Home** | The Kiyori-owned settings root. It displays only entries backed by a real page and state owner; unfinished download, player, reader, ad-blocking, backup, or permission-center pages do not appear early as placeholders. |
| **Browser Settings** | The single settings page shared by Settings Home, Browser Home, and the browser overlay. It edits `WebSessionHistoryStore` search engine and records, the Browser Runtime default Profile, and the existing desktop/mobile UA owner without creating parallel preferences. |
