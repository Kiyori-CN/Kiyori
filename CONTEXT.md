...
| **Browser Home** | The immersive Kiyori browser root. It hides the five-item Kiyori bottom navigation and presents the shared Browser Runtime through browser-owned address, tab, tool-drawer, and bottom-bar chrome. |
...
- Browser Home's bottom bar is ordered Back, Forward, Home, Tabs, and Toolbox. Tabs open an in-browser full-screen overview; history, bookmarks, downloads, userscripts, user-agent mode, and tab-close actions remain browser-owned overlays rather than Kiyori App Shell destinations.
