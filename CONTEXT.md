...
- Browser Home and the browser overlay are presentations of one Browser Runtime. Tabs, the active WebView, cookies, history, bookmarks, downloads, and userscripts are not copied between them. When the overlay is expanded, its top-left browser return only minimizes the overlay to the floating indicator; it never calls WebView history. Browser Home entered from AI Home records that source and returns to AI Home while restoring the indicator.
