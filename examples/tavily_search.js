...
const TavilySearch = (function () {
    ...
    async function executeRequest(spec, key) {
        ...
        if (spec.headers)
            builder = builder.headers(spec.headers);
        if (spec.body !== undefined)
            builder = builder.jsonBody(spec.body);
    ...
    function isKeyError(statusCode, path) {
        if (statusCode === 403 && path === "/logs")
            return false;
        return KEY_ERROR_STATUSES.indexOf(statusCode) >= 0;
    }
    async function requestWithRotation(spec, preferredKeyIndex) {
    ...
    async function createResearch(params) {
    ...
    async function usage(params) {
        ...
        if (params.project_id)
            headers["X-Project-ID"] = params.project_id;
    ...
    async function organizationUsage(params) {
