  ...
  "tools": [
    ...
    },
    {
      ...
      "parameters": [
        ...
        { "name": "query", "description": { "zh": "搜索关键词（默认 kiyori）", "en": "Search keyword (default: kiyori)." }, "type": "string", "required": false }
      ]
    }
  ]
}
...
async function main(params) {
    try {
        ...
        const query = String(params?.query || 'kiyori');
        ...
        if (!enableWrite) {
            ...
            const ts = Date.now();
            const testBranch = `kiyori-test-${ts}`;
            const testPath = `kiyori_test_${ts}.txt`;
            ...
            results.create_or_update_file = await run('create_or_update_file', async () => (0, contents_1.createOrUpdateFile)({
                ...
                repo,
                path: testPath,
                message: `kiyori test create file ${ts}`,
                content: `kiyori github tools self-test ${ts}`,
            ...
            results.patch_file_in_repo = await run('patch_file_in_repo', async () => (0, patch_1.patchFileInRepo)({
                ...
                repo,
                path: testPath,
                message: `kiyori test patch file ${ts}`,
                patch: `[START-REPLACE]\n[OLD]\nkiyori github tools self-test ${ts}\n[/OLD]\n[NEW]\nkiyori github tools self-test ${ts} (patched)\n[/NEW]\n[END-REPLACE]`,
                branch: testBranch
            }));
            results.delete_file = await run('delete_file', async () => {
                ...
                return (0, contents_1.deleteFile)({ owner, repo, path: testPath, message: `kiyori test delete file ${ts}`, branch: testBranch, sha });
            });
            ...
            if (!token) {
                ...
                results.create_issue = await run('create_issue', async () => (0, issues_1.createIssue)({ owner, repo, title: `kiyori self-test issue ${ts}`, body: `created by kiyori github tools self-test ${ts}` }));
            }
            if (!canIssue) {
                writeSkipped('comment_issue', 'Skipped: need GITHUB_TOKEN and issue_number (or at least one issue from list_issues).');
            }
            else {
                results.comment_issue = await run('comment_issue', async () => (0, issues_1.commentIssue)({ owner, repo, issue_number: issueNumber, body: `kiyori self-test comment ${ts}` }));
            }
            ...
            if (!token) {
                ...
                results.create_pull_request = await run('create_pull_request', async () => (0, pulls_1.createPullRequest)({ owner, repo, title: `kiyori self-test PR ${ts}`, head: prHead, base: prBase, body: `created by kiyori self-test ${ts}` }));
            }
            if (!token) {
