...
def merge_runs(paragraph) -> int:
    ...
    while index < len(runs) - 1:
        ...
        if not _is_pure_text_run(current) or not _is_pure_text_run(following):
            index += 1
            continue
    ...
    for match in reversed(matches):
        ...
        for run_index, (span_start, span_end) in enumerate(spans):
            if span_start <= start < span_end or (start == end == span_start):
                ...
                runs[run_index].text = head + replacement + tail_inside
                # 清空后续被命中的 run 文本
                for other in range(run_index + 1, len(spans)):
                    ...
                    if other_end > start:
                        runs[other].text = ""
                texts[run_index] = runs[run_index].text or ""
                replaced = True
                break
        if not replaced:
            continue
