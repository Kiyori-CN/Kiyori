...
exports.androidPathCandidates = androidPathCandidates;
exports.normalizeAndroidPath = normalizeAndroidPath;
...
function collectBusinessParams(spec, input) {
...
function declaresParam(spec, name) {
 ...
 */
function usesFilePaths(spec) {
    return ((Array.isArray(spec.inputPaths) && spec.inputPaths.length > 0) ||
...
function requireEnv(params, field) {
    const raw = (0, protocol_1.asText)(params[field]).trim();
    // 大小写归一：宿主系统层不做 enum 校验，若这里区分大小写就会出现
    // 「系统层放行、业务层报错」的两层职责错位。
    const value = raw.toLowerCase();
    if (value !== "android" && value !== "linux") {
        throw new Error(`E_INPUT_SCHEMA: ${field} 必须显式传入 android 或 linux，实际 ${raw || "(空)"}`);
    }
    return value;
}
function newTaskId() {
 ...
 */
function androidPathCandidates(raw) {
    const value = (0, protocol_1.asText)(raw).trim().replace(/\\/g, "/");
    if (!value) {
        throw new Error("E_INPUT_SCHEMA: 路径不能为空");
    }
    if (!value.startsWith("/")) {
        throw new Error(`E_INPUT_SCHEMA: Android 路径必须是绝对路径，实际收到 ${value}；` +
...
function normalizeAndroidPath(raw) {
 ...
 */
async function resolveAndroidSource(raw) {
    ...
    throw new Error(`E_PATH_INVALID: Android 源文件不存在，已探测 ${candidates.join(" / ")}；` +
 ...
 */
function assertFileOperation(result, operation, target) {
    const value = (result || {});
    const ok = value.successful === true || value.success === true;
    if (!ok) {
        const reason = (0, protocol_1.asText)(value.error).trim() ||
            (0, protocol_1.asText)(value.details).trim() ||
            (0, protocol_1.asText)(value.message).trim() ||
            "no diagnostic returned";
        throw new Error(`E_PATH_INVALID: ${operation} 失败 target=${target} reason=${reason}`);
    }
}
...
async function ensureRuntime() {
    ...
    assertFileOperation(await Tools.Files.copy(resourcePath, paths.zipPath, false, "android", "linux"), "复制 office 运行时资源", paths.zipPath);
    assertFileOperation(await Tools.Files.write(paths.bootstrapPath, BOOTSTRAP_SCRIPT, false, "linux"), "写入运行时引导脚本", paths.bootstrapPath);
...
async function verifyLocalFileProvider(paths) {
    ...
    if (created.exitCode !== 0)
        throw new Error("E_ENV_MISSING: 本地工作目录不可写");
    try {
        const read = await Tools.Files.read({ path: marker, environment: "linux" });
        if (read.content !== token)
            throw new Error("文件环境不一致");
    }
    catch (error) {
...
async function stageInput(paths, taskDir, value, env, slot) {
    if (env === "linux") {
        const source = (0, protocol_1.asText)(value).trim();
        const exists = await Tools.Files.exists(source, "linux");
        if (!exists || exists.exists !== true) {
            throw new Error(`E_PATH_INVALID: Linux 源文件不存在 path=${source}；请确认该文件位于 Linux 工作区`);
        }
...
async function verifyDeliveredSize(artifact, destination, environment) {
    ...
    if (actual !== artifact.bytes) {
        throw new Error(`E_PATH_INVALID: 交付产物大小不一致（疑似二进制损坏）target=${destination} ` +
...
async function createAndroidWorkspace(input) {
    ...
    for (const name of WORKSPACE_DIRECTORIES) {
        assertFileOperation(await Tools.Files.mkdir(`${target}/${name}`, true, "android"), "创建工作区目录", `${target}/${name}`);
    ...
    if (!exists || !exists.exists || input.overwrite === true) {
        assertFileOperation(await Tools.Files.write(agentsPath, WORKSPACE_AGENTS_MARKDOWN, false, "android"), "写入工作区规则", agentsPath);
...
async function runOfficeTool(spec, params) {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(taskId) ||
        (command === "office_workspace_clean" && !(0, protocol_1.asText)(input.task_id).trim())) {
    ...
    if (outcome.code === "E_TIMEOUT") {
        ...
        try {
            await changeTaskLease(paths, taskId, leaseToken, "release");
        }
        catch (error) {
...
async function changeTaskLease(paths, taskId, token, action) {
    ...
    const result = (0, protocol_1.parseEnvelope)(executed.output);
    if (!result.ok || executed.exitCode !== 0)
        throw new Error(`${result.error?.code ?? "E_ENGINE_FAILED"}: ${result.error?.message ?? "任务占用操作失败"}`);
}
async function runControlCommand(paths, spec, input) {
    const args = collectBusinessParams(spec, input);
    if (spec.command === "office_workspace_clean" && !(0, protocol_1.asText)(args.task_id).trim()) {
    ...
    try {
        ...
        outcome = { ...(0, protocol_1.toFailure)(spec.command, error) };
    }
    if (completed) {
        try {
            assertFileOperation(await Tools.Files.deleteFile(argsPath, false, "linux"), "清理本次管理参数", argsPath);
        }
        catch (error) {
        ...
        outcome.data = { ...(outcome.data ?? {}), control_args_path: argsPath };
    }
    return outcome;
}
async function runPreparedOfficeTool(spec, input, env, paths, taskId, leaseToken, deliveryRoot) {
    ...
    async function stageElements(value, slot) {
        if (Array.isArray(value)) {
            const staged = [];
            for (let index = 0; index < value.length; index++)
                staged.push(await stageElements(value[index], `${slot}-${index}`));
            return staged;
        }
    ...
    if (declaredOutputEnv && declaredOutputEnv !== "android" && declaredOutputEnv !== "linux") {
        throw new Error(`E_INPUT_SCHEMA: output_env 必须显式传入 android 或 linux，实际 ${declaredOutputEnv}`);
    }
    if (declaresParam(spec, "output_path")) {
        ...
        else if (declaredOutputEnv === "linux" && (0, protocol_1.asText)(input.output_path).trim()) {
            // 由 Python 侧直接写入调用方指定的 Linux 路径，仍需通过 allow_roots 校验。
            // 多产物命令的 output_path 是目录本身，必须整条加入白名单（去掉末段会
            // 把目录的父级当成允许根，既过宽也可能漏掉该目录）。
            const declaredPath = (0, protocol_1.asText)(input.output_path).trim();
            allowRoots.push(spec.outputKind === "multi" ? declaredPath : declaredPath.replace(/\/[^/]*$/, ""));
        }
        else {
    ...
    assertFileOperation(await Tools.Files.write(argsPath, JSON.stringify(payload), false, "linux"), "写入 office 参数文件", argsPath);
    ...
    if (!envelope.ok) {
        return (0, protocol_1.envelopeFailure)(envelope);
    }
    if (executed.exitCode !== 0) {
        return (0, protocol_1.toFailure)(command, new Error(`E_ENGINE_FAILED: 运行时退出码 ${executed.exitCode}，未发布产物`));
    }
    ...
    if (command === "office_render_preview" || command === "pdf_to_images") {
        ...
        for (const item of artifacts.slice(0, 8)) {
            if (item.env !== "android" && item.env !== "linux")
                throw new Error("E_PROTOCOL: 图片产物缺少有效环境");
            let content;
            try {
...
async function readOfficeGuide(input) {
...
async function executeSetup(paths, taskDir, taskId, plan) {
    ...
    const checked = (0, protocol_1.parseEnvelope)(probe.output);
    if (probe.exitCode !== 0)
        return (0, protocol_1.toFailure)("office_env_setup", new Error("E_ENGINE_FAILED: 安装复检进程异常退出，未确认安装完成"));
    if (!checked.ok)
        return (0, protocol_1.envelopeFailure)(checked);
    const tiers = checked.data?.tiers;
    const tier = tiers?.[`tier${plan.tier}`];
    const completed = Array.isArray(plan.components) && plan.components.every((name) => typeof name === "string" && tier?.components?.[name]?.available === true);
