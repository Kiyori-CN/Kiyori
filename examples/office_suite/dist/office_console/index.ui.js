...
function Screen(ctx) {
    const [tab, setTab] = ctx.useState("tab", "files");
    const [path, setPath] = ctx.useState("path", "");
    const [env, setEnv] = ctx.useState("env", "android");
    const [action, setAction] = ctx.useState("action", "read");
    const [pages, setPages] = ctx.useState("pages", "");
    const [outputEnv, setOutputEnv] = ctx.useState("outputEnv", "android");
    ...
    const [result, setResult] = ctx.useState("result", null);
    const [title, setTitle] = ctx.useState("resultTitle", "");
    const [details, setDetails] = ctx.useState("details", false);
    const [storage, setStorage] = ctx.useState("storage", null);
    const [plan, setPlan] = ctx.useState("cleanupPlan", null);
    const [planTier, setPlanTier] = ctx.useState("planTier", null);
    const [environment, setEnvironment] = ctx.useState("environment", null);
    const [picked, setPicked] = ctx.useState("pickedCopies", []);
    ...
    const text = (value, small = false) => ctx.UI.Text({ text: value, style: small ? "bodySmall" : "bodyMedium", color: small ? "onSurfaceVariant" : "onSurface" });
    const hint = (value) => text(value, true);
    const row = (children) => ctx.UI.Row({ spacing: 8, fillMaxWidth: true, verticalAlignment: "center" }, children);
    const secondary = (label, click, enabled = true) => ctx.UI.OutlinedButton({ onClick: click, enabled: !busy && enabled, shape: { cornerRadius: 10 } }, [text(label)]);
    const primary = (label, click, enabled = true) => ctx.UI.Button({ text: label, onClick: click, enabled: !busy && enabled, fillMaxWidth: true, shape: { cornerRadius: 10 } });
    const chip = (label, selected, click) => ctx.UI.FilterChip({ label: [ctx.UI.Text({ text: label, style: "labelLarge" })], selected, onClick: click, enabled: !busy, weight: 1 });
    const card = (label, children) => ctx.UI.Card({ fillMaxWidth: true, containerColor: "surface", elevation: 0,
    ...
    async function run(tool, params, label) {
        ...
        try {
            if (ctx.importPackage && ctx.isPackageImported && !(await ctx.isPackageImported("office")))
                await ctx.importPackage("office");
            if (ctx.usePackage)
                await ctx.usePackage("office");
            ...
            const raw = await ctx.callTool(name, params);
            const value = typeof raw === "string" ? JSON.parse(raw) : raw;
            if (!value || typeof value.success !== "boolean")
                throw new Error(t("工具未返回有效结果", "Invalid tool result"));
            setResult(value);
            if (value.success) {
                if (tool === "office_workspace_status")
                    setStorage(value.data ?? null);
                if (tool === "office_env_check")
                    setEnvironment(value.data ?? null);
                if (tool === "office_env_setup" && params.confirm !== true && Array.isArray(value.data?.commands)) {
                    setPlanTier(value.data?.tier ?? null);
                    setDetails(true);
                }
                if (tool === "office_workspace_clean") {
                    if (params.confirm !== true)
                        ...
                        setStorage(null); // 完成后重新扫描，不能把旧占用量显示为当前状态。
                }
            }
        }
        catch (error) {
    ...
    async function chooseFile() {
        ...
        try {
            ...
            if (!file?.path)
                throw new Error(t("选择器未返回可读副本，请输入文件路径", "Picker returned no readable copy; enter a path"));
        ...
        try {
            ...
            while (remaining.length) {
                ...
                if (!removed.successful)
                    throw new Error(removed.details ?? t("移除副本失败", "Could not remove copy"));
                remaining.shift();
                if (path === file.path)
                    setPath("");
            }
    ...
    async function open(item) {
        ...
        setBusy(true);
        try {
            const opened = await Tools.Files.open(item.path, "android");
            if (!opened.successful)
                throw new Error(opened.details);
        }
        catch (error) {
            setResult({ ...result, ...failure(error), artifacts: result?.artifacts });
        }
        finally {
            lock.current = false;
            setBusy(false);
    ...
    function executeFile() {
        ...
        if (action === "read")
            return run("office_read", { path: path.trim(), env, mode: "outline", max_chars: 12000 }, actionLabel);
        if (action === "validate")
            return run("office_validate", { path: path.trim(), env, strict: true }, actionLabel);
        if (action === "preview")
            return run("office_render_preview", { path: path.trim(), env, max_pages: 8, ...output, ...(pages.trim() ? { pages: pages.trim() } : {}) }, actionLabel);
        return run("office_convert", { from_path: path.trim(), env, engine: "libreoffice", to_format: "pdf", ...output }, actionLabel);
    }
    const navigate = (next) => { if (next === tab)
        return; setTab(next); setResult(null); setPlan(null); setPlanTier(null); setConfirmCopies(false); };
    const changeAction = (next) => { if (next === action)
        return; setAction(next); setOutputPath(""); setResult(null); };
    const nodes = [row([chip(t("文件操作", "Files"), tab === "files", () => navigate("files")), chip(t("存储管理", "Storage"), tab === "storage", () => navigate("storage")), chip(t("环境与帮助", "Help"), tab === "help", () => navigate("help"))])];
    if (tab === "files") {
    ...
    if (tab === "storage") {
        ...
        if (plan)
            nodes.push(card(t("确认清理范围", "Review cleanup"), [text(plan.path), text(plan.scope === "temporary" ? t("仅临时文件，保留 out 输出", "Temporary files only; preserve out outputs") : t("删除整个任务，包含 out 输出", "Delete the entire task, including out outputs")), text(`${plan.file_count} ${t("个文件", "files")} · ${bytes(plan.bytes)}`), hint(plan.sample_files.join("\n") || t("没有待清理文件", "No files to remove")),
        ...
        for (const task of storage?.tasks ?? [])
            nodes.push(card(task.task_id, [hint(task.path), text(`${bytes(task.bytes)} · ${t("临时", "Temp")} ${bytes(task.temporary_bytes)} · ${t("输出", "Outputs")} ${bytes(task.output_bytes)}`),
        ...
        if (storage && ((storage.offset ?? 0) > 0 || storage.next_offset != null))
            nodes.push(row([secondary(t("上一页", "Previous"), () => run("office_workspace_status", { offset: Math.max(0, (storage.offset ?? 0) - 20), limit: 20 }, t("存储扫描", "Storage scan")), (storage.offset ?? 0) > 0), secondary(t("下一页", "Next"), () => run("office_workspace_status", { offset: storage.next_offset, limit: 20 }, t("存储扫描", "Storage scan")), storage.next_offset != null)]));
    ...
    if (busy)
        nodes.push(ctx.UI.LinearProgressIndicator({ fillMaxWidth: true }));
    if (result) {
        ...
        for (const item of result.artifacts ?? [])
            nodes.push(card(item.path.split("/").pop() ?? t("产物", "Artifact"), [hint(`${item.env === "android" ? t("手机", "Phone") : "Ubuntu"}: ${item.path}`),
                ...(item.env === "android" ? [secondary(t(/\.(png|jpe?g)$/i.test(item.path) ? "查看预览图" : "打开文件", "Open file"), () => open(item))] : []),
                ...(/\.(docx|xlsx|xlsm|pptx|pdf)$/i.test(item.path) && ["android", "linux"].includes(item.env) ? [secondary(t("作为输入继续处理", "Use as input"), () => { setPath(item.path); setEnv(item.env); setTab("files"); setAction("read"); setPages(""); setResult(null); })] : [])]));
    }
