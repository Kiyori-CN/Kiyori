...
export async function buildExtraInfoAttachmentTags(
  ...
  if (settings.injectTime) {
    contentTasks.push(
      buildOptionalContent("time", text.attachmentTimeTitle, buildTimeContent, deadlineAt)
    );
  }

  if (settings.injectBattery) {
    contentTasks.push(
      buildOptionalContent("battery", text.attachmentBatteryTitle, buildBatteryContent, deadlineAt)
    );
  }

  if (settings.injectWeather) {
    contentTasks.push(
      buildOptionalContent("weather", text.attachmentWeatherTitle, buildWeatherContent, deadlineAt)
    );
  }
