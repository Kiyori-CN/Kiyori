...
type ZhipuPrimitive = string | number | boolean | null;
type ZhipuValue = ZhipuPrimitive | ZhipuObject | ZhipuValue[];
interface ZhipuObject {
    [key: string]: ZhipuValue | undefined;
}

interface ZhipuAttempt extends ZhipuObject {
    keyIndex: number;
    maskedKey: string;
    statusCode: number;
    message: string;
...
const ZhipuSearch = (function () {
    ...
    function parseJson(text: string): ZhipuValue {
        try {
            return JSON.parse(text) as ZhipuValue;
        } catch (error) {
    ...
    function errorCode(content: string): string {
        try {
            const parsed = JSON.parse(content) as { error?: { code?: string | number } };
    ...
    function isKeyError(statusCode: number, content: string): boolean {
        const code = errorCode(content);
        if (code === "1305") return false;
        if (code && KEY_ERROR_CODES.indexOf(code) >= 0) return true;
    ...
    async function execute(body: ZhipuObject, key: string): Promise<OkHttpResponse> {
    ...
    async function requestWithRotation(body: ZhipuObject): Promise<ZhipuObject> {
        const keys = configuredKeys();
        const start = nextStartIndex(keys.length);
        const attempts: ZhipuAttempt[] = [];
    ...
    async function runTool(name: string, action: () => Promise<ZhipuObject>): Promise<ZhipuObject> {
        try {
            return await action();
        } catch (error) {
    ...
    async function testOneKey(key: string, keyIndex: number, allKeys: string[]): Promise<ZhipuObject> {
    ...
    async function testKeys(): Promise<ZhipuObject> {
