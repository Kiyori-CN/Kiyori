...
interface CrossrefPublicationAuthor {
    name: string;
    orcid: string;
}

interface CrossrefPublicationRecord {
    source: string;
    doi: string;
    title: string;
    subtitle: string;
    authors: CrossrefPublicationAuthor[];
...
interface CrossrefResult {
    success: boolean;
    message: string;
    data?: CrossrefPublicationRecord[] | CrossrefPublicationRecord;
    total?: number;
    count?: number;
    offset?: number;
...
const CrossrefSearch = (function () {
    ...
    function referenceLimit(value: number | undefined): number {
    ...
    function compactText(value: string | undefined, maximumLength: number): { text: string; truncated: boolean } {
    ...
    function dateText(value: CrossrefDateParts | undefined): string {
        const parts = value?.["date-parts"]?.[0];
        if (!parts || parts.length === 0 || !Number.isFinite(parts[0])) return "";
    ...
    function publicationAuthor(author: CrossrefAuthor): CrossrefPublicationAuthor {
    ...
    function compactReference(reference: CrossrefReference): CrossrefReference {
        return {
            DOI: reference.DOI,
            key: reference.key,
    ...
    function publicationRecord(work: CrossrefWork, includeReferences: boolean, maximumReferences: number): CrossrefPublicationRecord {
        ...
        const record: CrossrefPublicationRecord = {
            source: "Crossref",
            doi: work.DOI ?? "",
    ...
    async function request(path: string, params: Record<string, string>): Promise<CrossrefResponse | CrossrefResult> {
        ...
        const payload = response.json() as CrossrefResponse;
    ...
    function worksResult(payload: CrossrefResponse, resultOffset: number): CrossrefResult {
