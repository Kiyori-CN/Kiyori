...
function normalizeAgentUrl(input) {
    ...
    if (/%(?![a-f\d]{2})/i.test(prefix) || /(?:^|\/)(?:\.|\.\.)(?:\/|$)/.test(prefix))
        throw new Error("地址路径无效");
    return `${match[1].toLowerCase()}://${host}${port === null ? "" : `:${port}`}${prefix}`;
}
function validateConnectionConfig(baseUrl, token, shell = "powershell", timeout = "30000") {
    const normalized = normalizeAgentUrl(baseUrl);
    if (!token.trim() || /[\r\n]/.test(token))
        throw new Error("请填写有效的访问令牌");
    if (!["powershell", "pwsh", "cmd"].includes(shell))
        throw new Error("Shell 必须为 powershell、pwsh 或 cmd");
    const timeoutMs = Number(timeout);
    if (!Number.isInteger(timeoutMs) || timeoutMs < 1000 || timeoutMs > 600000)
        throw new Error("超时必须为 1000–600000 毫秒的整数");
