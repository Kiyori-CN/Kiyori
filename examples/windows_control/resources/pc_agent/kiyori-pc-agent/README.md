...
本版协议为 `1.1.0`，需要配套 Windows 工具包。源码源于 Operit，Kiyori 定制连接、界面、鉴权和文件行为。
...
3. 自动打开的页面选择“局域网”或“FRP / 远程连接”，保存设置。修改监听地址或端口后重新双击启动器，并使用新打开的页面。
...
- 监听 IP 失效时，已有启动恢复流程会明确显示临时本地模式，由用户应用推荐 IP 后重启；不将其显示为手机已连接。
