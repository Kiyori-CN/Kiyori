...
当前未发布的手机工具包与 PC Agent 产品版本为 `1.0.0`，认证后的执行协议独立为 `1.1.0`。
手机读取认证响应的 `protocolVersion`，支持稳定版 `1.1.x`，产品版本无需与协议版本相同。
未提供协议字段的历史电脑端只接受原有 `version=1.1.x`；旧 `1.0.0` 服务仍明确拒绝。
