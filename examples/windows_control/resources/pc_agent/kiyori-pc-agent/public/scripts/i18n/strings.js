...
const RESOURCES = {
  en: {
    ...
    message: {
      ...
      inputPaused: "Input paused; it will not be resent. Check the PC state, then refresh the session list to send new input.",
      sessionMissing: "Session no longer exists (the service may have restarted). Stale input was cleared. Create or select a session: {sessionId}",
      sessionClosing: "Termination requested; waiting for the session to exit: {sessionId}",
      sessionReadFailed: "Session update failed; input paused: {error}",
  ...
  zh: {
    ...
    message: {
      ...
      inputPaused: "输入已暂停，不会自动重发。请先核对电脑执行状态，再刷新会话列表以发送新输入。",
      sessionMissing: "会话已不存在（服务可能已重启），失效输入已清空。请新建或选择会话：{sessionId}",
      sessionClosing: "已请求终止，正在等待进程退出：{sessionId}",
      sessionReadFailed: "会话更新失败，输入已暂停：{error}",
