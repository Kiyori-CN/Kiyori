...
const codeRunner = (function () {
    ...
    function createTempToken(prefix) {
        tempPathSequence += 1;
    ...
    async function executeFromHome(command, timeoutMs = DEFAULT_COMMAND_TIMEOUT_MS) {
    ...
    async function ensurePersistentNodeWorkspace() {
        ...
        if (!hasPackageJson.output.includes("OK")) {
            await writeTextFile(`${NODE_WORKSPACE_DIR}/package.json`, `{
  ...
  "private": true
    ...
    function createHereDocMarker(content) {
...
func main() {
  fmt.Println("Go运行正常")
    ...
    async function ensureRustConfigured() {
        ...
        let rustCheckResult = await executeTerminalCommand(buildSubshellCommand("/tmp", `${RUST_TOOLCHAIN_ENV} && rustc --version && cargo --version`));
        ...
        if (rustCheckResult.output.includes("no default is configured")) {
            ...
            // 再次检查
            rustCheckResult = await executeTerminalCommand(buildSubshellCommand("/tmp", `${RUST_TOOLCHAIN_ENV} && rustc --version && cargo --version`));
            if (rustCheckResult.exitCode === 0 && !hasError(rustCheckResult.output)) {
