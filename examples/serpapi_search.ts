...
type SerpPrimitive = string | number | boolean | null;
type SerpValue = SerpPrimitive | SerpObject | SerpValue[];
interface SerpObject {
    [key: string]: SerpValue | undefined;
}

interface SerpAttempt extends SerpObject {
    keyIndex: number;
    maskedKey: string;
    statusCode: number;
    message: string;
...
const SerpApiSearch = (function () {
    ...
    function encodeQuery(params: SerpObject): string {
        const parts: string[] = [];
    ...
    function searchParams(engine: string, params?: SerpObject): SerpObject {
        const result: SerpObject = { engine };
    ...
    function parseJson(text: string): SerpValue {
        try {
            return JSON.parse(text) as SerpValue;
        } catch (error) {
    ...
    function keyError(statusCode: number, content: string): boolean {
        if ([401, 403, 429].indexOf(statusCode) >= 0) return true;
    ...
    async function requestWithKey(path: string, query: SerpObject, key: string): Promise<OkHttpResponse> {
        const fullQuery: SerpObject = {};
    ...
    async function requestWithRotation(path: string, query: SerpObject, expectsJson: boolean, preferredKeyIndex?: number): Promise<SerpObject> {
    ...
    async function runTool(name: string, action: () => Promise<SerpObject>): Promise<SerpObject> {
        try {
            return await action();
        } catch (error) {
    ...
    function searchPath(output?: string): { path: string; json: boolean } {
        const normalized = (output ?? "json").toLowerCase();
        if (normalized === "json") return { path: "/search.json", json: true };
        if (normalized === "html") return { path: "/search", json: false };
        if (normalized === "markdown" || normalized === "md") return { path: "/search.md", json: false };
        throw new Error("output must be json, html, or markdown.");
    }

    function archivePath(searchId: string, output?: string): { path: string; json: boolean } {
        const base = "/searches/" + encodeURIComponent(searchId);
        const normalized = (output ?? "json").toLowerCase();
        if (normalized === "json") return { path: base + ".json", json: true };
        if (normalized === "html") return { path: base, json: false };
        if (normalized === "pixel") return { path: base + ".json_with_pixel_position", json: true };
        throw new Error("output must be json, html, or pixel.");
    }

    async function search(params: { engine: string; params?: SerpObject; output?: string }): Promise<SerpObject> {
    ...
    async function getSearch(params: { search_id: string; key_index: number; output?: string }): Promise<SerpObject> {
    ...
    async function locations(params: { query?: string; limit?: number }): Promise<SerpObject> {
        const query: SerpObject = {};
        if (params.query) query.q = params.query;
        if (params.limit !== undefined) query.limit = params.limit;
        return runTool("locations", () => requestWithRotation("/locations.json", query, true));
    }

    async function account(): Promise<SerpObject> {
        return runTool("account", () => requestWithRotation("/account.json", {}, true));
    }

    async function testOneKey(key: string, keyIndex: number, allKeys: string[]): Promise<SerpObject> {
    ...
    async function testKeys(): Promise<SerpObject> {
