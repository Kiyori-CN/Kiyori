...
const linuxSshTools = (function () {
    ...
    function validateWindowName(value) {
        ...
        if (name && !/^[a-zA-Z0-9_-]{1,64}$/.test(name))
            throw new Error("window_name must contain 1..64 letters, digits, underscores or hyphens");
        return name;
    }
    ...
    async function resolveSshConfig(params, options) {
        ...
        if (/[\s\u0000-\u001f\u007f/@]/.test(username) || username.startsWith("-"))
            throw new Error("Invalid SSH username");
        if (password.includes("\u0000") || /[\u0000-\u001f\u007f]/.test(privateKeyPath))
    ...
    async function runLocalHiddenCommand(command, timeoutMs, scope, config = null) {
        ...
        if (!Number.isInteger(result.exitCode))
            throw new Error("SSH execution returned no confirmed exit code; do not retry a modifying command blindly");
        if (result.outputTruncated)
            throw new Error("SSH output was truncated; request a smaller range. Do not repeat modifying commands.");
        return {
            sessionId: "",
            executorKey,
    ...
    async function ensureLocalSshDependencies(config, runner) {
        ...
        if (result.exitCode !== 0 || result.timedOut)
            throw new Error(`Local SSH tools unavailable. Install SSH tools in the local terminal environment first. exitCode=${result.exitCode}; output=${result.output}`);
    }
    ...
    async function ensureRemoteTmux(config, allowInstall = false) {
        ...
        const result = await runRemoteCommandHidden(config, buildRemoteShellCommand(installScript, [], false), 240000, "tmux");
    ...
    async function readRemoteFileContent(config, path, lineStart, lineEnd, useSudo) {
        ...
        if (!/^(?:[0-9a-fA-F]{2})*$/.test(hex))
            throw new Error("Unexpected SSH file response; file data was not decoded");
    ...
    async function writeRemoteFileContent(config, path, content, appendMode, useSudo) {
        ...
        const command = buildRemoteShellCommand(script, [path], useSudo);
        const result = await runRemoteCommandWithLocalStdinHidden(config, content, command, config.timeoutMs, "fs", false);
        if (result.exitCode !== 0 || result.timedOut) {
    ...
    async function linux_ssh_exec(params) {
        return await runTool(async () => {
            ...
            const config = await resolveStoredSshConfig(params);
            const timeoutMs = params?.timeout_ms ?? config.timeoutMs;
            const result = await runRemoteCommandWithLocalStdinHidden(config, command, buildRemoteUserShellCommand(), timeoutMs, "remote", true);
            const success = !result.timedOut;
    ...
    async function linux_ssh_tmux_run(params) {
        return await runTool(async () => {
            ...
            const result = await runRemoteCommandWithLocalStdinHidden(config, command, buildRemoteShellCommand(script, [targetWindow, workdir, windowName], false), config.timeoutMs, "tmux", true);
