...
async function kiyori_editor(params: { query?: string }) {
  try {
    const { query } = params ?? {};
    complete({
      success: true,
...
async function resolve_toolpkg_source(rawSourcePath: string): Promise<ToolPkgResolvedSource> {
  ...
  if (sourceType === "directory") {
    ...
    const tempExtractDir = path_join(
      KIYORI_CLEAN_ON_EXIT_DIR,
      `kiyori_editor_toolpkg_extract_${safe_debug_file_stem(lowerBaseName.replace(/\.toolpkg$/i, ""), "toolpkg")}_${Date.now()}`
    );
...
function pick_context_summary_fields(config: ModelConfigResultItem) {
  return {
