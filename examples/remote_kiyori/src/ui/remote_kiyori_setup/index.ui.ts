...
import { resolveRemoteKiyoriSetupI18n, type RemoteKiyoriSetupI18n } from "../../i18n";

const REMOTE_KIYORI_PACKAGE_NAME = "remote_kiyori";
const REMOTE_KIYORI_TOOL_CONFIGURE = "remote_kiyori_configure";
const REMOTE_KIYORI_TOOL_TEST_CONNECTION = "remote_kiyori_test_connection";

const KEY_BASE_URL = "REMOTE_KIYORI_BASE_URL";
const KEY_TOKEN = "REMOTE_KIYORI_TOKEN";
const KEY_TIMEOUT_MS = "REMOTE_KIYORI_TIMEOUT_MS";
...
function resolveText(): RemoteKiyoriSetupI18n {
  const rawLocale = getLang();
  const locale = rawLocale.trim().toLowerCase();
  const preferredLocale = locale.startsWith("en") ? "en-US" : "zh-CN";
  return resolveRemoteKiyoriSetupI18n(preferredLocale);
}
...
function toErrorText(error: unknown): string {
  console.error("[remote_kiyori_setup] Operation failed", error instanceof Error ? error.name : typeof error);
...
function readTimeoutMs(raw: string): number {
  const timeout = Number(raw.trim() || "60000");
  if (!Number.isSafeInteger(timeout) || timeout <= 0 || timeout > 2147483642000) {
...
function parseToolRecord(result: unknown): Record<string, unknown> {
  ...
  if (typeof result === "string") {
    ...
    if (!raw) {
      throw new Error("Empty remote tool response");
    }
    try {
      ...
      toErrorText(error);
      throw new Error("Invalid remote tool response");
    }
  }
  throw new Error("Expected a remote tool response object");
}
...
async function resolveToolName(
  ...
  throw new Error(`Cannot resolve remote tool: ${packageName}:${toolName}`);
}

async function ensureImportedAndUsed(ctx: ComposeDslContext, packageName: string, text: RemoteKiyoriSetupI18n): Promise<void> {
...
export default function Screen(ctx: ComposeDslContext): ComposeNode {
  ...
  const callRemoteTool = async (toolName: string, params?: Record<string, unknown>): Promise<Record<string, unknown>> => {
    const packageName = REMOTE_KIYORI_PACKAGE_NAME;
    await ensureImportedAndUsed(ctx, packageName, text);

    const resolved = await resolveToolName(ctx, packageName, toolName);
    // 工具名由宿主解析一次；调用失败后重复保存会造成不明确的配置副作用。
    const result = await ctx.callTool(resolved, params);
  ...
  const checkConnectionByTool = async (): Promise<void> => {
    ...
    const result = await callRemoteTool(REMOTE_KIYORI_TOOL_TEST_CONNECTION, {
      timeout_ms: readTimeoutMs(timeoutMsState.value)
    });
  ...
  const saveConfig = async (shouldTest: boolean): Promise<void> => {
    ...
    try {
      const result = await callRemoteTool(REMOTE_KIYORI_TOOL_CONFIGURE, {
        base_url: baseUrl,
        token,
        timeout_ms: readTimeoutMs(timeoutMs)
      });
