...
export const REMOTE_KIYORI_SETUP_ZH_CN: RemoteKiyoriSetupI18n = {
  title: "远程 Kiyori 配置",
  subtitle: "让当前 Kiyori 通过局域网 HTTP API 调用另一台设备上的 Kiyori。",
  topBanner:
    "先在目标设备的 Kiyori 设置里启用“外部 HTTP 调用”，再把该设备页面里显示的局域网地址和 Bearer Token 填到这里。",
  configCardTitle: "连接配置",
  configCardSubtitle:
    "保存后，当前 Kiyori 就能把聊天请求转发到另一台设备上的 Kiyori。推荐填写完整的 http://IP:端口。",
  baseUrlLabel: "远程 Kiyori Base URL",
  ...
  errorBaseUrlRequired: "请填写远程 Kiyori 的 Base URL",
  errorTokenRequired: "请填写远程 Kiyori 的 Bearer Token",
  importPackageFailed: "导入 remote_kiyori 包失败",
  toolCallFailedPrefix: "调用工具失败：",
  statusSaved: "配置已保存并启用 remote_kiyori。",
  statusErrorPrefix: "操作失败：",
  packageNotEnabled: "包未导入或配置不完整，请先保存连接配置。",
  exampleCardTitle: "使用方式",
  exampleCardSubtitle:
    "导入并启用后，当前 Kiyori 可以直接把任务转发到另一台设备。常用入口是 remote_kiyori_chat。",
  examplePromptTitle: "推荐场景",
  examplePromptBody:
    "当另一台设备拥有本机没有的文件、应用、权限或上下文时，让当前 Kiyori 调 remote_kiyori_chat 去协作。",
  exampleParamsTitle: "remote_kiyori_chat 参数示例",
  exampleParamsBody:
