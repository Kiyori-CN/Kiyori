...
const SemanticScholarSearch = (function () {
    ...
    function fields(value: string | undefined): string {
    ...
    async function getPaper(params: SemanticPaperParams): Promise<SemanticResult> {
        ...
        const payload = await requestJson<SemanticPaper>(`/paper/${encodeURIComponent(params.paper_id.trim())}`, { fields: fields(params.fields) });
        if ("success" in payload) return payload;
