export type LocalizedStrings = {
  ...
  probePassed: string;
  noStatus: string;
  compatibilityNotRequired: string;
  compatibilityMissing: string;
  compatibilityStale: string;
  compatibilityValid: string;
  compatibilityFailed: string;
  compatibilityMissingHint: string;
  compatibilityStaleHint: string;
  compatibilityFailedHint: string;
  probeErrorCode: string;
  probeHttpStatus: string;
  probeFailureMessage: string;
  providerErrorType: string;
  providerErrorCode: string;
  providerRequestId: string;
  evidenceMode: string;
  evidenceModeCitationsAndSources: string;
...
const ZH: LocalizedStrings = {
  ...
  probeWarning:
    "仅用于 RESPONSES_RELAY_STRICT 的可选诊断。环境变量配置有效后普通 search 可直接使用；此按钮会真实调用远端接口，可能产生费用。",
  ...
  compatibilityMissingHint:
    "当前严格中转绑定尚未运行兼容探测。配置有效后普通 search 可直接使用；探测只用于验证中转站的 Web Search 证据合同。",
  compatibilityStaleHint:
    "endpoint、模型、认证、headers、reasoning、联网模式或响应 schema 已变化。普通 search 仍按当前配置执行；如需更新诊断记录，可重新运行兼容探测。",
  compatibilityFailedHint:
    "最近一次兼容探测未满足完整 Responses Web Search 证据合同。普通 search 仍会返回真实结果或结构化错误；修正中转站后可再次运行探测。",
  probeErrorCode: "错误码",
...
const EN: LocalizedStrings = {
  ...
  probeWarning:
    "Optional diagnostics for RESPONSES_RELAY_STRICT. Ordinary search is available as soon as the environment is valid; this button sends a real request and may incur charges.",
  ...
  compatibilityMissingHint:
    "This strict relay binding has not been probed. Ordinary search is available when the environment is valid; the probe only verifies the relay's Web Search evidence contract.",
  compatibilityStaleHint:
    "The endpoint, model, authentication, headers, reasoning, web access, or response schema changed. Ordinary search still uses the current valid binding; rerun the probe only to refresh diagnostics.",
  compatibilityFailedHint:
    "The last compatibility probe did not satisfy the complete Responses Web Search evidence contract. Ordinary search still returns the real result or a structured error; correct the relay before probing again.",
...
export function strings(): LocalizedStrings {
  const language =
    typeof getLang === "function" ? getLang().trim().toLowerCase() : "";
