...
const ZH: LocalizedStrings = {
  ...
  probeTitle: "中转站兼容探测",
  probeWarning:
    "只在 RESPONSES_RELAY_STRICT 模式使用。此按钮会真实调用配置的远端接口，可能产生费用；不会自动触发。",
  ...
  compatibilityMissingHint:
    "当前严格中转绑定尚未运行兼容探测。普通 search 会返回 RELAY_PROBE_REQUIRED；请确认费用后点击下方按钮。",
  compatibilityStaleHint:
    "endpoint、模型、认证、headers、reasoning、联网模式或响应 schema 已变化。必须重新运行兼容探测。",
  compatibilityFailedHint:
    "最近一次真实兼容探测未满足完整 Responses Web Search 证据合同。修正中转站后重新运行探测。",
  probeErrorCode: "错误码",
...
const EN: LocalizedStrings = {
  ...
  probeWarning:
    "Use only with RESPONSES_RELAY_STRICT. This button sends a real request to the configured endpoint and may incur charges; it is never triggered automatically.",
  ...
  compatibilityMissingHint:
    "This strict relay binding has not been probed. Ordinary search returns RELAY_PROBE_REQUIRED until you explicitly run the potentially billable probe below.",
  compatibilityStaleHint:
    "The endpoint, model, authentication, headers, reasoning, web access, or response schema changed. Run the compatibility probe again.",
  compatibilityFailedHint:
    "The last real compatibility probe did not satisfy the complete Responses Web Search evidence contract. Correct the relay and run the probe again.",
