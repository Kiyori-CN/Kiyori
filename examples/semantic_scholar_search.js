...
const SemanticScholarSearch = (function () {
    ...
    function fields(value, defaults) {
        ...
        selected.split(",").forEach((rawField) => {
            ...
            if (field === "")
                throw new Error("fields must not contain empty entries.");
            if (!ALLOWED_FIELDS.has(field))
                throw new Error(`Unsupported Semantic Scholar field: ${field}.`);
            if (seen.has(field))
                return;
            seen.add(field);
            unique.push(field);
    ...
    async function getPaper(params) {
        ...
        const payload = await requestJson(`/paper/${encodeURIComponent(params.paper_id.trim())}`, {
